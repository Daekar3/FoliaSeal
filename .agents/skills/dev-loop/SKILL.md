...
After your first pass is implemented, spawn an "explorer-light" subagent to review `docs/ARCHITECTURE.md` `docs/SPEC.md`, and any other specifications or requirements in `docs/` to ensure that the implementation from this slice aligns with the overall architecture and requirements of the project.  If the slice is high-risk, spawn a second "explorer-light" subagent to provide an additional review. If there are any discrepancies or areas for improvement, use $write-execplan to create a new child ExecPlan to address these issues, implement the plan in accordance with `## Execute the Plan` above, do the compliance review in accordance with `## Review Compliance with Requirements`,and repeat the development loop as needed until the slice implementation is compliant with the requirements.
