"""Qt-free Acceptance evidence result models and decision logic."""
...
class EvidenceMatrixKind(StrEnum):
...
@dataclass(frozen=True)
class SignedAcceptanceMatrixCounters:
    scenario_count: int | None
...
@dataclass(frozen=True)
class SignedAcceptanceMatrixResult:
    name: str
    passed: bool
    errors: tuple[str, ...]
    artifacts_dir: str
    summary_json_path: str
    counters: SignedAcceptanceMatrixCounters
...
@dataclass(frozen=True)
class SignedAcceptanceEvidenceResult:
    passed: bool
    summary_markdown_path: str
    generated_assets: dict[str, str]
    matrix_results: tuple[SignedAcceptanceMatrixResult, ...]
    errors: tuple[str, ...]
...
def normalize_matrix_result(
    *, kind: EvidenceMatrixKind, summary: Mapping[str, Any]
    ...
    if kind is EvidenceMatrixKind.PREVIEW:
...
def matrix_summary_row(
    ...
    return SignedAcceptanceMatrixResult(
        name=name,
        passed=not errors,
        errors=tuple(errors),
        artifacts_dir=artifacts_dir,
...
def render_evidence_markdown(evidence: SignedAcceptanceEvidenceResult) -> str:
    lines = [
        "# Acceptance Signed Acceptance Evidence",
        "",
