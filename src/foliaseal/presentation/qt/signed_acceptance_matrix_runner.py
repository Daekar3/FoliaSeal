"""Qt-backed signed-acceptance matrix runner boundary for Acceptance QA."""
...
from foliaseal.presentation.qt.signed_acceptance_lifecycle import (
    QtSignedAcceptanceLifecycle,
    SignedAcceptanceLifecyclePort,
)
...
BuildSigningExecutor = Callable[..., Any]
BuildDummyTimestamper = Callable[[], Any]
LoadPageCount = Callable[..., int]
BuildWorkspace = Callable[..., AcceptanceHarnessWorkspacePort]
...
LifecycleFactory = Callable[[Any], SignedAcceptanceLifecyclePort]
ArtifactPortFactory = Callable[[], EvidenceArtifactPort]


@dataclass(frozen=True)
class SignedAcceptanceMatrixRunnerDeps:
    ...
    build_signing_executor: BuildSigningExecutor
...
@dataclass(frozen=True)
class SignedAcceptanceMatrixRunner:
    ...
    def run(
        ...
        *,
        ...
        sign_executor = self.deps.build_signing_executor(
            timestamper_factory=(
        ...
        lifecycle_factory = self.deps.lifecycle_factory or QtSignedAcceptanceLifecycle
        lifecycle = lifecycle_factory(bindings)
        try:
            lifecycle.start(
                title=f"FoliaSeal Acceptance Signed Acceptance Matrix - {source_path.name}"
            )
            ...
            for scenario in scenarios:
                ...
                results.append(
                    ...
                    if isinstance(scenario_result, SignedAcceptanceScenarioResult)
