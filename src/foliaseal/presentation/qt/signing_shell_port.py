...
class QtSigningWorkspaceSessionPort:
    ...
    def flush_pending_keyboard_placement(self) -> None:
        self.shell_widget.flush_pending_keyboard_placement()
    ...
    def selected_signature_preset_id(self) -> str | None:
        getter = getattr(self.shell_widget, "selected_signature_preset_id", None)
        return getter() if callable(getter) else None

    def selected_appearance_profile_id(self) -> str | None:
        getter = getattr(self.shell_widget, "selected_appearance_profile_id", None)
        return getter() if callable(getter) else None

    def selected_placement_profile_id(self) -> str | None:
        getter = getattr(self.shell_widget, "selected_placement_profile_id", None)
        return getter() if callable(getter) else None
