...
Finally, update `README.md`, `docs/ExecPlans/phase3_parallel_plan.md`, and if necessary `docs/SPEC.md` so the repository’s current truth is explicit: appearance parity is now expected to be debuggable through saved structural snapshots and autonomous parity tests, not only through manual image inspection.
...
This work belongs in `README.md`, `docs/ExecPlans/phase3_parallel_plan.md`, and optionally `docs/SPEC.md` if the instrumentation expectations need a clearer statement there. Run the same validation commands again after the documentation edits to ensure nothing drifted.
...
- the new ExecPlan itself at `docs/ExecPlans/autonomous_preview_pdf_parity_instrumentation_execplan.md`
