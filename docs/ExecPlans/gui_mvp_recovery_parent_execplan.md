...
- [ ] `docs/ExecPlans/gui_document_review_usability_execplan.md` must land before the staged-flow work can truthfully present review/navigation as a completed part of the main workflow.
- [ ] `docs/ExecPlans/gui_text_selection_mode_execplan.md` must land before the staged-flow work can claim document text review and copy behavior are usable.
- [ ] `docs/ExecPlans/gui_signing_flow_guidance_execplan.md` can begin after the two document-usability child plans are understood, but should not be considered complete until they land because the staged flow must route through the real review affordances.
- [ ] `docs/ExecPlans/gui_certificate_and_preset_clarity_execplan.md` can run in parallel with the staged-flow guidance work, but it should not be treated as complete until the clearer wording and settings-directory affordances are rechecked in the live GUI.
- [ ] `docs/ExecPlans/gui_preset_first_shell_reduction_execplan.md` depends on the staged-flow guidance decisions because the shell cannot safely shrink the inline editor until the new primary flow is explicit.
- [ ] `docs/ExecPlans/gui_preset_first_shell_reduction_execplan.md` also depends on `docs/ExecPlans/gui_certificate_and_preset_clarity_execplan.md` and on a verified fallback path for manual refinement or reusable-object editing so the shell does not hide or delete the only working refinement path allowed by `docs/SPEC.md`.
...
- [ ] Validate the implemented `gui_certificate_and_preset_clarity_execplan.md` slice in the live GUI, then mark it complete if the new wording and directory-picker affordances read correctly in context.
- [ ] Execute `gui_signing_flow_guidance_execplan.md` as the next development slice after that validation to make the staged workflow explicit before broader shell reduction.
- [ ] Execute `gui_preset_first_shell_reduction_execplan.md`.
- [ ] Run one end-to-end GUI walkthrough against a representative PDF and reconcile `docs/ARCHITECTURE.md`, `README.md`, and any acceptance notes with the resulting product behavior.
