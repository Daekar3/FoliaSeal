...
- [x] (2026-09-08) Completed installed Cinnamon/X11 acceptance against package `f800e6c...` built from commit `8903512c9`: held movement, Ctrl resize, Ctrl+Shift resize, responsiveness, one-step Undo/Redo, and sustained CPU/disk behavior all passed in the user's installed retest.
- [x] (2026-09-07) Reconciled the behavior children and older stability plan; authoritative validation is 199 focused tests and 1660 full-suite tests with 20 skips and one existing warning. Fresh package rebuild/install and rendered Cinnamon/X11 human acceptance are now closed by the 2026-09-08 installed retest.
...
History child implementation evidence (2026-09-07): viewer projection no longer clears history, pointer and removal operations commit exactly once, keyboard replay remains projection-only, unchanged panel refreshes preserve both stacks, and the shell forwards placement history capabilities to AppFrame. Focused real-Qt coverage includes existing-handle cancellation, Pan cancellation, AppFrame pointer edit Undo/Redo, menu removal Undo/Redo, Delete Undo, lifecycle reset and same-page render counters. Installed package `f800e6c...` from commit `8903512c9` passed the corresponding Cinnamon/X11 acceptance.

Strict-interface remediation evidence (2026-09-07): the composition now invokes the current viewer builder contract once, without a TypeError retry that strips keyboard callbacks. Runtime and shell-port Gate 10 history methods call required typed methods directly. Fakes were migrated to implement those callbacks and history operations. Ruff and `git diff --check` pass; installed package `f800e6c...` from commit `8903512c9` passed the corresponding Cinnamon/X11 acceptance.

AppFrame contract evidence (2026-09-07): `_sync_edit_history_actions` now invokes the typed session `can_undo_placement()` and `can_redo_placement()` methods directly whenever a workspace is active. The final optional-method fallback was removed so a missing history implementation cannot silently disable Gate 10. The app-frame and dependent integration tests pass 86 tests; installed package `f800e6c...` from commit `8903512c9` passed the corresponding Cinnamon/X11 acceptance.

Keyboard autorepeat finding (2026-09-07): individual Arrow adjustment passed the installed retest, but holding adjustment keys during resize caused significant CPU activity. Read-only tracing identified `apply_signature_rect_placement`, properties-panel `load_from_workflow`, setup/coordinator refresh, signing readiness, and possibly PKCS#12 readiness parsing as repeated-work candidates. The typed batching repair and configured disposable PKCS#12 counters addressed those paths; the installed `f800e6c...` retest confirms responsive movement/resizing, one-step history, and no unacceptable sustained CPU/disk behavior.

Governing-document review (2026-09-07): `docs/SPEC.md`, `docs/SCHEMAS.md`, and `docs/UI_SPEC.md` were reviewed against the implementation and this family. No governing-document change is required: the repaired source follows the existing Pan/Place/Text, keyboard-focus, cancelable-drag, visible-overlay, and undoable-placement contract. README validation remains the documented repository command surface; no README change is required for this behavior slice.
...
The family is complete: real menu activation, keyboard move/resize, held-button Escape, idle Escape to Pan, pointer edit Undo/Redo, Remove/Undo, repeated held-key adjustment, bounded callback work, one-step history, and the installed Cinnamon/X11 retest all have explicit passing evidence. Retain user-reported signing passes and record any new signing regression separately.
