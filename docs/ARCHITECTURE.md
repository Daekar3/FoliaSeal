...
2. `FoliaSealAppFrame` creates a `QMainWindow`, mounts the no-document landing panel, and installs File/Open, Settings/Application settings, Settings/Create certificate, Settings/Import certificate, and Settings/Manage certificate configurations menu actions. The landing panel provides direct `Open a PDF…` and `Manage Signature Library…` actions while the document-dependent actions remain in their closed state.
...
6. `SigningWorkspaceHost.open(path)` builds the existing Qt signing shell through `QtSigningWorkspaceFactory.create()`, whose unchanged `SigningWorkspaceBootstrap` is translated once into the typed `QtSigningWorkspaceComposition` boundary, and returns a typed `WorkspaceHandle`. The host/lifecycle transition composes the candidate, mounts it through `QtWorkspaceMount`, publishes the new handle, then disposes the old widget. A mount failure disposes only the candidate and leaves the prior workspace active.
   ...
   After a successful open or placeholder mount, `FoliaSealAppFrame` projects the corresponding immutable `WorkspaceActionState` onto the existing QActions; selection toggles update only the projection's checked field after the maintenance port returns. Lifecycle and maintenance routing remain unchanged.
