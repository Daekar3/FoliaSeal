...
- Known constraints: Widgets use dynamic PySide6 imports and many test doubles. `app_frame.py` owns a first-pass `QMainWindow` wrapper with File/Open, Settings/Application settings, Settings/Import certificate, and Settings/Manage certificate configurations actions; the settings action opens an app-wide settings dialog for default directories, the import action opens a PKCS#12 import dialog, and the management action can rename, edit notes for, or delete `CertificateConfiguration` records without deleting managed certificate files. `signing_shell.py` is a large module with control dataclasses, helper functions, properties panel, workspace widget, and shell adapter in one file. The shell can select and apply existing certificate configurations and can refresh its selector after app-frame certificate catalog changes, but certificate creation, export/backup, managed certificate deletion, and secure password storage are still pending. The signing shell consumes `AppSettings` for output-path defaults but no longer edits app-wide directory settings directly.
...
- Location: `src/foliaseal/infra/config/schemas.py`, `src/foliaseal/infra/config/app_settings_storage.py`, `src/foliaseal/infra/config/profile_storage.py`, `src/foliaseal/infra/config/certificate_storage.py`, `src/foliaseal/application/certificate_import.py`, `src/foliaseal/application/signing_material_resolver.py`
- Responsibility: Serialize, deserialize, validate, load, and save trust/timestamp configuration plus reusable signing-object catalogs; import existing PKCS#12 certificates into app-managed storage.
- Owns: `AppSettings`, `AppSettingsStore`, `TrustProfile`, `TimestampPolicy`, `ManagedCertificate`, `CertificateConfiguration`, `CertificateCatalog`, `CertificateCatalogStore`, `SigningMaterial`, `CertificateSigningMaterialResolver`, `AppearanceProfile`, `PlacementProfile`, reference-only `SignaturePreset`, `ResolvedSignaturePreset`, `SignaturePresetCatalog`, `SignaturePresetCatalogStore`.
...
- Main entry points: `AppSettings.default()`, `AppSettings.from_dict()`, `AppSettingsStore.load_settings()`, `AppSettingsStore.save_settings()`, `ManagedCertificate.from_dict()`, `CertificateConfiguration.from_dict()`, `CertificateCatalog.from_dict()`, `CertificateCatalog.remove_configuration_by_id()`, `CertificateCatalogStore.load_catalog()`, `CertificateCatalogStore.delete_configuration_by_id()`, `CertificateImportService.import_pkcs12()`, `CertificateSigningMaterialResolver.resolve_by_configuration_id()`, `AppearanceProfile.from_dict()`, `PlacementProfile.from_dict()`, `SignaturePreset.from_dict()`, `SignaturePresetCatalog.from_dict()`, `SignaturePresetCatalog.resolve_preset()`, `SignaturePresetCatalog.preset_names()`, `SignaturePresetCatalog.preset_named()`, `SignaturePresetCatalog.upsert_preset()`, `SignaturePresetCatalog.remove_preset()`, `SignaturePresetCatalogStore.load_catalog()`, `save_catalog()`, `save_preset()`, `delete_preset()`.
- Known constraints: App settings storage uses `${XDG_CONFIG_HOME:-~/.config}/FoliaSeal/settings.json` and returns home-directory defaults when missing or blank. The Qt app frame uses the default open directory for File/Open, owns the app-wide settings dialog for editing default directories, and passes settings into the signing shell; the save-output dialog uses the default output directory. The profile store still uses the historical user-visible `Signature Profiles/profiles.json` path, but the JSON shape now separates `appearance_profiles`, `placement_profiles`, and `signature_presets`. Certificate configuration storage uses `${XDG_DATA_HOME:-~/.local/share}/FoliaSeal/Certificates/certificates.json` plus a `Managed/` directory for app-owned PKCS#12 files. Existing PKCS#12 files can be imported through `CertificateImportService` and the app-frame import dialog; import validates the file/password, copies the file into managed storage, creates one `ManagedCertificate`, and creates one `CertificateConfiguration` without saving the password. The app frame can rename, edit notes for, and delete `CertificateConfiguration` records by stable id, but deletion does not remove `ManagedCertificate` records or managed PKCS#12 files. Passwords are not stored in ordinary JSON; saved-password lookup is a resolver protocol seam with no concrete OS credential adapter yet. Obsolete signature-preset compatibility wrappers such as `profile_names()`, `profile_named()`, `save_profile()`, and `delete_profile()` have been removed; `appearance_profile_named()` and `placement_profile_named()` remain because they address canonical profile objects.
- Status: Confirmed by code and tests.
...
- Producer: `CertificateCatalogStore.save_catalog()`, `CertificateCatalogStore.delete_configuration_by_id()`, and `CertificateImportService.import_pkcs12()` through the app-frame certificate dialogs.
...
- Error behavior: missing or blank file loads as an empty catalog; invalid JSON raises `ConfigValidationError`; deleting an unknown configuration id raises `KeyError`; import failures raise `CertificateImportError`; resolver failures raise `SigningMaterialResolutionError` with user-actionable messages.
...
| Managed certificate configuration | Certificate import service, app-frame certificate management dialog, tests, or Qt shell selection | imported PKCS#12 -> managed certificate record + certificate configuration -> optional app-frame rename/notes/delete of configuration record -> resolver -> runtime signing material -> signing draft | XDG data dir under `FoliaSeal/Certificates/certificates.json`; PKCS#12 files under `FoliaSeal/Certificates/Managed/` | `CertificateCatalog` JSON plus PKCS#12 files | JSON stores secret references only; no plain certificate passwords. Qt app frame can import existing PKCS#12 files, rename/edit notes/delete configuration records, and Qt shell can select existing configurations; create/export/managed certificate deletion and secure password storage remain pending. |
...
| Full certificate management is not implemented yet. | Users can import existing PKCS#12 files, rename/edit notes/delete resulting certificate configurations, and select configurations, but they cannot yet create, export, back up, delete managed certificate files, or save passwords for managed certificates from the GUI. | Tests and lower-level stores can create catalogs; the app frame owns first-pass import/configuration management and the shell consumes configurations through the resolver. | Expand the certificate management surface for create/export/managed certificate deletion and add a real credential-store adapter for saved passwords. |
