...
- Known constraints: The shell still owns overwrite confirmation and error emission routing. The sidebar now owns widget mutation for returned state, and the coordinator intentionally returns immutable snapshots so shell and sidebar tests can focus on adapter behavior instead of duplicated state policy.
- Status: Confirmed by code and tests.
