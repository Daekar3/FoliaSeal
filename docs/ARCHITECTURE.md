...
- Known constraints: Widgets use dynamic PySide6 imports and many test doubles. `app_frame.py` owns a first-pass `QMainWindow` wrapper with File/Open, Settings/Application settings, Settings/Import certificate, and Settings/Manage certificate configurations actions; the settings action opens an app-wide settings dialog for default directories, the import action opens a PKCS#12 import dialog, and the management action can rename, edit notes for, or delete `CertificateConfiguration` records, and can delete unreferenced `ManagedCertificate` records/files. `signing_shell.py` is a large module with control dataclasses, helper functions, properties panel, workspace widget, and shell adapter in one file. The shell can select and apply existing certificate configurations and can refresh its selector after app-frame certificate catalog changes, but certificate creation, export/backup, and secure password storage are still pending. The signing shell consumes `AppSettings` for output-path defaults but no longer edits app-wide directory settings directly.
...
- Producer: `CertificateCatalogStore.save_catalog()`, `CertificateCatalogStore.delete_configuration_by_id()`, `CertificateCatalogStore.delete_managed_certificate_by_id()`, and `CertificateImportService.import_pkcs12()` through the app-frame certificate dialogs.
...
- Error behavior: missing or blank file loads as an empty catalog; invalid JSON raises `ConfigValidationError`; deleting an unknown configuration or managed certificate id raises `KeyError`; deleting a managed certificate referenced by a certificate configuration raises `ConfigValidationError`; import failures raise `CertificateImportError`; resolver failures raise `SigningMaterialResolutionError` with user-actionable messages.
...
| Managed certificate configuration | Certificate import service, app-frame certificate management dialog, tests, or Qt shell selection | imported PKCS#12 -> managed certificate record + certificate configuration -> optional app-frame rename/notes/delete of configuration record -> optional app-frame delete of unreferenced managed certificate/file -> resolver -> runtime signing material -> signing draft | XDG data dir under `FoliaSeal/Certificates/certificates.json`; PKCS#12 files under `FoliaSeal/Certificates/Managed/` | `CertificateCatalog` JSON plus PKCS#12 files | JSON stores secret references only; no plain certificate passwords. Qt app frame can import existing PKCS#12 files, rename/edit notes/delete configuration records, delete unreferenced managed certificate files, and Qt shell can select existing configurations; create/export and secure password storage remain pending. |
...
| Full certificate management is not implemented yet. | Users can import existing PKCS#12 files, rename/edit notes/delete resulting certificate configurations, delete unreferenced managed certificate files, and select configurations, but they cannot yet create, export, back up, or save passwords for managed certificates from the GUI. | Tests and lower-level stores can create catalogs; the app frame owns first-pass import/configuration/managed-certificate deletion and the shell consumes configurations through the resolver. | Expand the certificate management surface for create/export and add a real credential-store adapter for saved passwords. |
