...
| `phase3_signing_backend.py` still mixes concrete pyHanko adapter code with compatibility-layer visible-signature layout helpers. | Harder to navigate and test at a single public boundary. | Structural reservation sizing and background-layout construction now live in `visible_signature_layout.py`, while the backend retains compatibility helper names plus rendered-fit fallback and fit-validation logic. | Finish moving the remaining fit/rejection helpers behind the layout boundary, then reduce private-helper test reliance after coverage is equivalent. |
