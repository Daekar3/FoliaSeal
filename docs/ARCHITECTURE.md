...
- Known constraints: `FoliaSealAppFrame` owns one reusable-library dialog instance and opens it modelessly; `ReusableObjectLibraryDialog` builds one horizontal three-column `QSplitter` with catalog navigation, a Presets-first/searchable master list, and a details column. `SignatureLibrarySession` owns selection/search projection and the transactional name draft; the dialog delegates reusable-object writes to `ReusableSigningObjects`, including typed duplicate and pin commands, and its explicit `save_detail()` boundary commits the current name transaction through the rename command before refreshing. Preset Create/Edit now open `SignaturePresetEditorDialog`, a document-independent modal Save/Cancel editor that writes stable appearance/placement/certificate references through `SavePreset`; it never mutates a live signing draft. AppFrame routes typed certificate pin, rename, and delete callbacks to certificate-catalog policy/manager services, so configured and unconfigured certificate rows are actionable. AppFrame persists the selected catalog and Name A-Z/Z-A order through `AppSettings.ui.library_last_catalog` and `library_sort`. Appearance editing remains a future document-independent editor, while placement create/edit uses the document-independent numeric editor. Certificate create/import/configure operations remain in Settings certificate surfaces; reason/location defaults, expiration sorting, dirty-detail prompts, and active-placement invalidation prompts remain explicit deferred boundaries.
- Status: Confirmed by code and tests.
