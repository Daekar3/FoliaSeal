...
- Responsibility: Adapt transactional placement state to Qt and expose create/edit entry points from the modeless Settings Library.
...
- Known constraints: The editor is intentionally document-independent for Library creation and shows captured visible page dimensions/rotation; edits use the profile's existing fixed page metadata. The Library remains modeless and delegates appearance/preset create/edit to the active signing workflow while placement create/edit uses this dedicated editor.
- Status: Confirmed by code and tests.
...
| Placement editing is numeric-only and the Library is still a single-selector management surface. | Users can create and edit fixed-page placement profiles, but cannot drag a rectangle with visual pointer handles; the mixed appearance/placement/preset catalog is harder to scan than the intended dedicated management layout. | `PlacementEditorState`/`PlacementEditorSession` keep edits transactional, and `PlacementProfileEditorDialog` exposes exact numeric fields plus Save/Cancel; `ReusableObjectLibraryDialog` provides typed create/edit callbacks and refreshes after writes. | Add visual pointer/drag handles after the placement interaction contract is settled, then evolve the Library toward the deferred three-column appearance/placement/preset layout without duplicating application policy in Qt. |
