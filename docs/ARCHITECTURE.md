...
1. The app frame composes a `CertificateSigningMaterialPort` from the certificate repository and secret provider; `build_qt_signing_shell()` receives the application port and repository protocol, not a managed directory or secret provider for resolution.
