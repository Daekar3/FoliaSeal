...
5. The session delegates signing-draft rule enforcement to the coordinator, asks `certificate_configuration_name_for_preset()` for preset password prompt labels, owns manual password retry/cancel/cache policy plus preset save/delete orchestration and programmatic appearance dirty clearing, and returns `SigningSetupSelectionOutcome` for certificate/preset selection while returning plain `SignaturePropertiesViewState` for the other setup verbs.
