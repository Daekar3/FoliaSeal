...
- Main entry points: `Phase3HarnessSessionRunner.run()` and `Phase3Composition.capture()`/`Phase3Harness.capture()`.
- Known constraints: The helper keeps the interactive shell flow Qt-bound, but the collected session result must stay stable enough for later payload assembly and report finalization. The runner now consumes `Phase3HarnessWorkspaceSnapshot` and adapts it back to the legacy dict payload expected by `phase3_harness_capture_assembler.py`; `phase3_harness_workspace.py` still intentionally does not absorb the session lifecycle itself. `Phase3Composition` is headless-first: preview and signed operations are lazy and the interactive Qt graph is installed only by `with_interactive_qt()` (and remains lazy until `capture()`), so validation and matrix callers do not construct the capture session. Operation-local dependency bundles inject profile stores, lifecycles, renderers, artifact ports, and signing executors; no redundant gateway or private forwarding wrapper is part of the contract.
- Status: Confirmed by code and tests.
...
| `Phase3Composition` / `Phase3Harness` | `presentation/qt/phase3_harness.py` | Headless-first composition and stable three-verb adapter for Phase 3 harness modes. | Lazy preview/signed matrix operations, optional lazy interactive capture, and explicit verbs that consume application-layer requests. | `default_headless()` avoids constructing the interactive Qt graph; `with_interactive_qt()` opts into capture. Operation-local runner dependency bundles own injectable factories. |
...
|---|---|---|---|
| The Phase 3 harness still spans several broad presentation helpers after the command-pipeline refactor. | Understanding or changing a full harness run still requires bouncing between the Qt adapter, session runner, matrix runners, scenario executors, and the live testing seam. | `Phase3Composition` keeps headless matrix construction separate from lazy interactive capture; workspace/session/matrix modules own narrower runtime boundaries and operation-local dependencies. | Continue deepening the harness in narrow slices. |
...
| 2026-08-01 | Reconciled the headless-first Phase 3 composition slice. | Documented lazy `Phase3Composition` matrix/capture wiring, operation-local dependency injection, preserved CLI/JSON/artifact contracts, and removal of redundant matrix gateways and private forwarding wrappers. |
