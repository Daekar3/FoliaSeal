...
`FoliaSealAppFrame` establishes a `1100x700` logical-pixel minimum for the main window before applying the selected application palette. `MainWindowGeometry` now persists the clamped x/y position, minimum-safe width/height, and maximized state; `AppUiSettings.rail_width` persists the bounded canvas/rail divider width, and the adapter restores/captures both before display and after the event loop. Library minimum/geometry, full monitor/DPI-aware rerendering, and toolbar overflow/persistence remain explicitly deferred.
