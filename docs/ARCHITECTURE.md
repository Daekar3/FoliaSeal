...
| `src/foliaseal/presentation/qt/signing_workspace_compatibility_surface.py` | Transitional widget-export and testing seam for the signing workspace shell. | `SigningWorkspaceTestingAdapter` is independently constructed from runtime, properties-panel, and live-result-reader dependencies; `SigningWorkspaceLegacyWidgetExports` is the idempotent Qt-local installer for historical widget aliases and destruction disposal. The transitional facade only composes those two pieces; new app-frame and harness callers use the typed bundle/testing port, while legacy widget exports remain quarantined for compatibility. |
...
- Current lifecycle source of truth: `SigningWorkspaceHost` owns the active `WorkspaceHandle`; the frame does not store `_current_workspace`, `_current_shell_port`, or a `WorkspaceCompatibilityState`. Frame `current_*` properties are projections of `host.active()`. The host/lifecycle path owns central-widget mounting and `SigningWorkspacePort` no longer has `widget()`; `compat_surface` and `testing_adapter` remain an intentional Qt-local compatibility/testing boundary, while interactive and signed-acceptance Phase 3 runners now have a typed factory path that reuses one `SigningWorkspaceBundle`. The action-state projection does not add lifecycle or maintenance routing: `SigningWorkspaceHost` remains the sole compose/mount/publish/dispose owner, and text-selection/copy commands continue through the existing maintenance port.
...
- Known constraints: The bundle is a capability record, not a service locator; callers must select the narrow capability they need. `WorkspaceViewPort` is intentionally opaque so lifecycle code cannot inspect child widgets. `compat_surface` remains a one-way transitional facade over the Qt-local legacy-export installer; no new app-frame or harness code should reach through it. Production construction requires the explicit testing adapter even though tests may opt into `allow_missing_testing` for minimal fakes. The compatibility-bridge ExecPlan is the retirement gate for removing the remaining dynamic aliases.
...
| Date | Change | Reason |
|---|---|---|
| 2026-08-06 | Threaded the AppFrame-owned `ReusableSigningObjects` identity through the production Qt workspace graph. | Workspace open, bootstrap, composition, properties panel, and coordinator now share one service; legacy catalog/store kwargs remain only at the low-level shell compatibility edge, and the dedicated atomic phase3 nomenclature plan remains queued for a complete rename migration. |
| 2026-08-06 | Deepened the reusable-signing catalog boundary. | `ReusableSigningObjects` now owns immutable snapshots, typed indexes, duplicate/reference validation, and atomic preset composition; `DefaultSignaturePropertiesCoordinator` no longer caches or persists a second catalog or reaches into a private repository. Legacy constructor inputs are one-way adapters, and the no-store certificate fallback no longer consults XDG/home path policy from the coordinator. |
