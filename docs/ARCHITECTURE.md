...
The current typed View command surface also includes `Select Text`, `Fit Page` (`Ctrl+0`), and `Fit
Width` (`Ctrl+Shift+0`) in addition to Previous/Next Page. Fit actions are document-dependent and
route through `SigningWorkspaceSessionPort.fit_page_view()` / `fit_width_view()`; the concrete
`SigningWorkspaceWidget` delegates those verbs to the scroll-aware PDF viewer. The viewer derives
fit geometry from its actual scroll-area viewport and rendered page snapshot, starts its first
visible view in Fit Page, clamps interactive zoom to 10%–800%, and treats ordinary wheel input as
vertical panning while Ctrl+wheel performs zoom. The toolbar exposes the same two fit actions, and
the page/overlay coordinate snapshot remains page-local during fit and pan.
