...
- Owns: `ReusableSigningObjects`, `ReusableCatalogSnapshot`, `PresetSelection`, `ReusableObjectKind`, `ReusableObjectRef`, `ReusableObjectSummary`, `ReusableObjectsView`, typed save/rename/delete commands, and the narrow `CatalogRepository` protocol.
...
- Known constraints: The session starts on `Presets`; rows are filtered by display name/details and sorted case-insensitively. `refresh()` reprojects reusable objects and the latest injected certificate catalog, clearing stale selections without changing document state. Selecting a row copies its name into a transactional draft; `detail_dirty` reflects edits, and only a caller's successful command followed by `commit_detail()` marks it clean. Closing/canceling the modeless window discards this session-local state.
- Status: Confirmed by code and tests.
...
- Known constraints: `FoliaSealAppFrame` owns one reusable-library dialog instance and opens it modelessly; `ReusableObjectLibraryDialog` builds one horizontal three-column `QSplitter` with catalog navigation, a Presets-first/searchable master list, and a details column. `SignatureLibrarySession` owns selection/search projection and the transactional name draft; the dialog delegates reusable-object writes to `ReusableSigningObjects` and refreshes injected certificate rows from `CertificateCatalogStore.load_catalog()` each refresh. The certificates catalog is projection-only here: certificate mutations remain in dedicated Settings certificate surfaces. Appearance and preset create/edit remain callbacks into the active signing workflow, while placement create/edit uses the document-independent numeric editor. Nested rich editors, certificate mutations, pin/duplicate actions, dirty-detail prompts, and Library-specific preference persistence are explicitly deferred; the current Save/Cancel name boundary does not mutate a live signing draft.
- Status: Confirmed by code and tests.
...
- Format: JSON object with `schema_version`, `default_output_directory`, `default_open_directory`, `linux_packaging_channel`, and `ui`. `AppSettings.ui_settings` exposes the typed `AppUiSettings` projection; its known `appearance_mode` and optional `main_window_geometry` values are merged back into the mapping without dropping unknown UI keys. `main_window_geometry` stores integer `x`, `y`, `width`, `height`, and boolean `maximized`, with dimensions never below the 1100x700 application minimum.
...
| Placement editing is numeric-only and the Library's detail surface is intentionally shallow. | Users can create and edit fixed-page placement profiles, but cannot drag a rectangle with visual pointer handles; rich nested editing and several Library management affordances are not yet available. | `PlacementEditorState`/`PlacementEditorSession` keep edits transactional, and `PlacementProfileEditorDialog` exposes exact numeric fields plus Save/Cancel. `ReusableObjectLibraryDialog` now provides the AppFrame-owned modeless three-column topology, typed create/edit callbacks, searchable rows, injected certificate projection/refresh, and a transactional name draft. | Add visual pointer/drag handles after the placement interaction contract is settled. Add nested rich editors, certificate mutations, pin/duplicate, dirty-detail prompts, and Library-specific preference persistence only behind explicit contracts; do not duplicate application policy in Qt. |
...
| Date | Change | Reason |
|---|---|---|
| 2026-08-10 | Added the Signature Library session and current three-column modeless topology. | `SignatureLibrarySession` owns document-independent catalog/search/selection state, searchable master rows, injected managed-certificate/configuration projections and refresh, and the transactional name draft boundary. `FoliaSealAppFrame` owns one modeless `ReusableObjectLibraryDialog`; its horizontal `QSplitter` is catalog navigation, master/search rows (Presets first), and details. Nested editors, certificate mutations, pin/duplicate, dirty prompts, and Library preferences remain explicit deferred debt. |
