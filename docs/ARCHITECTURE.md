...
| `src/foliaseal/presentation/qt/app_frame_profile_library.py` | Qt adapter for the modeless Signature Library. | Builds the AppFrame-owned three-column `QSplitter` (catalog navigation, master/search rows, details), delegates state to `SignatureLibrarySession`, supports non-persisting first-use Presets focus, gates destructive mutations behind an explicit Yes/No question, notifies the active signing shell after reusable-object saves, refreshes and reselects a retained certificate after successful configuration, and owns the nested Appearance and Signature Preset detail-editor sessions with suspended parent selection and typed dirty resolution. |
...
- Owns: `ManagedSignatureImageStore`, `SignatureImageInspection`, import errors, supported-format/size limits, EXIF orientation handling, sRGB/RGBA normalization, optional alpha flattening, explicit large-image optimization approval, and atomic managed PNG writes under `signature-images/` beside the profile catalog.
...
- Main entry points: `ManagedSignatureImageStore.inspect()` and `.import_image()`.
...
| `SignatureAppearance` | `domain/models.py` | Normalized visible signature style/content settings. | field bindings, layout template, stamp position, `image_prominence`, `preserve_image_alpha`, font/box style, image path. | Validates field order and binding rules; new image fields default to primary prominence and alpha preservation. |
...
- Format: JSON object with `schema_version`, `appearance_profiles`, `placement_profiles`, and `signature_presets`. Appearance entries persist `pinned`; their `SignatureAppearance` payloads include `image_prominence` (`supporting`, `balanced`, or `primary`) and `preserve_image_alpha`, while `SignaturePreset` entries are reference-only, persist `pinned`, and can point to optional certificate configuration, appearance profile, and placement profile ids.
- Validation: `SignaturePresetCatalog.from_dict()` and nested schema constructors reject malformed shape/types/duplicates; older appearance payloads missing `image_prominence` or `preserve_image_alpha` read as `primary` and `true` respectively.
...
| Image prominence is now explicit in reusable appearances, but the legacy `None` representation still means `PRIMARY` at backend/preview compatibility edges. | New callers must distinguish an absent image from primary image prominence without changing established `SigningBackendAppearance` and preview payload shapes. | Domain/catalog/workflow/layout contracts carry `SignatureImageProminence`; `sign_pdf_use_case.py` and `signing_preview_renderer.py` omit `PRIMARY` when adapting to the historical optional field, while layout handles image-only/text-only cases explicitly. | Retire the compatibility `None`/`PRIMARY` translation only with a coordinated backend, preview, schema, and acceptance-contract migration; preserve preview/signing parity meanwhile. |
...
| Date | Change | Reason |
|---|---|---|
| 2026-08-10 | Added reusable signature-image prominence and managed import boundaries. | `SignatureAppearance` now persists `SignatureImageProminence` and `preserve_image_alpha` with backward-read defaults; `ManagedSignatureImageStore` validates and normalizes user images to atomic catalog-local PNGs, and AppFrame injects it through the Library into nested Appearance/Preset editors. Layout reserves prominence-specific image space and handles absent/image-only inputs, while the historical `PRIMARY`/`None` backend-preview compatibility translation remains documented debt. |
