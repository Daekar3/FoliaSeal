...
| `src/foliaseal/application/phase3_evidence_orchestrator.py` | Canonical application-owned boundary exposing explicit evidence verbs and capture validation. | Delegates explicit methods to the injected service port; intentionally has no Qt, PyHanko, Pillow, TSA, filesystem-artifact, or presentation imports. |
...
| `src/foliaseal/presentation/qt/evidence_runner_factories.py` | Neutral lazy factories for interactive capture, preview matrices, and signed-acceptance matrices. | Each factory builds its concrete Qt/runtime graph only on first request; the three lifecycles remain separate. |
...
| `src/foliaseal/presentation/qt/` | Qt viewer/signing widgets and manual/automated harnesses. | Uses dynamic PySide6 imports so tests can use fakes; app-frame, signing-shell, workspace, lifecycle, reporting, matrix, snapshotter, render, runner-factory, and artifact responsibilities are documented in their focused modules below. `phase3_interactive_capture.py` owns the interactive capture contract, runner, JSON normalization, and artifact policy; `evidence_runner_factories.py` owns neutral lazy runner construction; `phase3_matrix_operations.py` owns the lazy matrix holder; `evidence_artifacts.py` owns neutral preview/signed summary publication; `phase3_harness.py` remains the concrete Qt composition root. The application service/orchestrator remains the caller boundary. |
...
- Location: `src/foliaseal/application/qa_*`, `src/foliaseal/application/phase2_evidence.py`, `src/foliaseal/application/phase3_evidence_service.py`, `src/foliaseal/presentation/qt/phase*_harness.py`, `src/foliaseal/presentation/qt/phase3_interactive_capture.py`, `src/foliaseal/presentation/qt/phase3_signed_acceptance_evidence.py`, `artifacts/`
...
- Owns: `Phase3ValidationRequest`, `Phase3EvidenceServicePort`, explicit `capture()`, `preview_matrix()`, `signed_acceptance_matrix()`, `signed_acceptance_evidence()`, `validate()`, and conversion of validation requests into the service validation DTO.
...
- Main entry points: the explicit orchestrator verbs, `for_pdf()`, and `orchestrator_for_service()`.
...
- Key collaborators: `phase3_interactive_capture.py`, `phase3_harness.py`, `phase3_harness_capture_assembler.py`, `phase3_harness_reporting.py`, `build_qt_signing_shell()`, `SigningDraftWorkflow`, `ViewerWorkflow`.
- Main entry points: `Phase3HarnessSessionRunner.run()` and the explicit capture factory in `phase3_interactive_capture.py`; `phase3_harness.py` remains the concrete builder/composition root for the runner dependency graph.
- Known constraints: The helper keeps the interactive shell flow Qt-bound, but the collected session result must stay stable enough for later payload assembly and report finalization. The runner now consumes `Phase3HarnessWorkspaceSnapshot` and adapts it back to the legacy dict payload expected by `phase3_harness_capture_assembler.py`; `phase3_harness_workspace.py` still intentionally does not absorb the session lifecycle itself. Matrix callers use `EvidenceMatrixOperations`, while interactive capture is installed separately and remains lazy until requested. Operation-local dependency bundles inject profile stores, lifecycles, renderers, artifact ports, and signing executors.
- Status: Confirmed by code and tests.

### Phase 3 interactive capture boundary

- Location: `src/foliaseal/presentation/qt/phase3_interactive_capture.py`
- Responsibility: Own the structured interactive capture result, stable JSON normalization, interactive runner choreography, narrow artifact-path policy, and lazy capture factory.
- Owns: `Phase3HarnessCapture`, `Phase3InteractiveHarnessRunner`, `Phase3InteractiveCaptureArtifactPolicy`, `build_interactive_evidence_capture_runner()`, and `jsonable_capture()`.
- Does not own: concrete Qt/render/signing dependency construction, which remains in `phase3_harness.py`, or application-level request dispatch, which remains in `phase3_evidence_service.py` and `phase3_evidence_orchestrator.py`.
- Key collaborators: `phase3_harness.py`, `phase3_harness_session_runner.py`, `phase3_harness_capture_assembler.py`, `phase3_harness_reporting.py`, and `phase3_signed_acceptance_evidence.py`.
- Main entry points: `Phase3InteractiveHarnessRunner.run()` and `build_interactive_evidence_capture_runner()`.
...
- Owns: `build_interactive_evidence_runner()`, `build_preview_evidence_runner()`, `build_signed_acceptance_evidence_runner()`, and request-callable lazy operation factories.
...
- Location: `src/foliaseal/presentation/qt/phase3_matrix_operations.py`
- Responsibility: Hold the two dependency-light lazy matrix callables injected into the evidence service.
- Owns: `EvidenceMatrixOperations` and `build_evidence_matrix_operations()`.
- Known constraints: Concrete preview and signed runners are constructed only when selected; this module imports no Qt, PIL, pyHanko, rendering, or harness composition root.
- Status: Confirmed by code and tests.
...
- Main entry points: `Phase3PreviewMatrixRunner.run()` and `EvidenceMatrixOperations.preview`.
...
- Main entry points: `Phase3SignedAcceptanceMatrixRunner.run()` and `EvidenceMatrixOperations.signed_acceptance`.
...
- Key collaborators: `phase3_interactive_capture.py`, `phase3_harness.py`, `phase3_harness_capture_assembler.py`, `qa_evidence_contract.py`, `build_phase3_checklist_results_markdown()`, `Phase3HarnessCapture`.
...
| `Phase3ValidationRequest` | `application/phase3_evidence_orchestrator.py` | Read-only capture-validation input. | summary JSON path. | Explicit validation verb converts this to the service DTO. |
| `Phase3EvidenceOrchestrator` | `application/phase3_evidence_orchestrator.py` | Canonical application boundary over the injected service port. | service port and explicit verb requests. | No generic tagged dispatcher; returns service result objects unchanged. |
...
| `Phase3HarnessCapture` | `presentation/qt/phase3_interactive_capture.py` | Structured acceptance harness result. | preview/request/signing/evidence fields. | JSON output is validated by evidence contract; concrete dependency composition remains in `phase3_harness.py`. |
...
- Source files: `src/foliaseal/presentation/qt/phase3_interactive_capture.py`, `src/foliaseal/presentation/qt/phase3_harness.py`, `src/foliaseal/application/qa_evidence_contract.py`, `artifacts/`.
...
- Format: Explicit orchestrator verbs accept the existing service request DTOs; `Phase3ValidationRequest` remains read-only. `Phase3EvidenceSession` binds `pdf_path`, `certificate_path`, `passphrase`, and default `artifacts/phase3` paths, with per-call artifact overrides.
...
- Validation: Each explicit method forwards its typed request; `validate()` delegates to `Phase3EvidenceServicePort.validate_harness_capture()`.
...
|---|---|---|---|
| The Phase 3 harness still spans several broad presentation helpers after the command-pipeline refactor. | Understanding or changing a full harness run still requires bouncing between the Qt adapter, session runner, matrix runners, scenario executors, and the live testing seam. | `EvidenceMatrixOperations` keeps lazy matrix construction separate from the explicit interactive runner factory; workspace/session/matrix modules own narrower runtime boundaries and operation-local dependencies. | Continue deepening the harness in narrow slices. |
...
| The Phase 3 command pipeline still has separate service execution and orchestrator modules. | A caller must understand the explicit application verbs versus injected runner effects when extending evidence operations. | `phase3_evidence_core.py` and `phase3_evidence_ports.py` keep result truth and effects explicit while the orchestrator remains thin. | Add new operations through typed request/result and port changes together; do not reintroduce a generic dispatcher or gateway. |
...
| Date | Change | Reason |
|---|---|---|
| 2026-08-02 | Reconciled the evidence nomenclature slice. | Deleted the duplicate `evidence_gateways.py` layer and tagged orchestrator dispatcher/types; current code uses explicit orchestrator verbs, `evidence_runner_factories.py`, and `EvidenceMatrixOperations` while preserving CLI commands, persisted `Phase3*` DTOs, JSON fields, summary paths, and artifact paths. |
