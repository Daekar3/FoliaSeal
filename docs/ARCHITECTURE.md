...
- Key collaborators: `visible_signature_semantics.py`, `phase3_signing_backend.py`, `signing_preview_renderer.py`, `presentation/qt/signing_shell.py`, `presentation/qt/phase3_harness.py`, `visible_signature_color.py`, and `visible_signature_layout_adapters.py`. Backend and Qt evidence callers consume the public policy facade for neutral spacing, margins, reservation, and fit checks; rendered-ink measurement remains backend/materializer-owned.
