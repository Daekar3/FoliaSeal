...
| `src/foliaseal/presentation/qt/app_frame_workspace_action_state.py` | Qt-free app-frame projection of workspace QAction policy. | `WorkspaceActionState` and its pure closed/open/selection-result constructors describe whether Save, Save As, Close, Previous Page, Next Page, Back, Forward, text selection, and Copy selected text should be enabled or checked. The projection owns policy only; `FoliaSealAppFrame` remains responsible for mutating the concrete QActions. |
...
- Known constraints: The shell still owns overwrite confirmation and the boundary still owns error emission routing. The sidebar owns widget mutation for returned state, and the coordinator intentionally returns immutable snapshots so shell and sidebar tests can focus on adapter behavior instead of duplicated state policy. `recommended_action` identifies the currently supported sign, reopen, and preserved-artifact recovery affordances. A preserved-copy reopen carries an explicit untrusted mode; signing remains blocked until every-signature verification and the known DocMDP permission projection allow a later approval. The coordinator now owns a non-cancellable begin/complete lifecycle with truthful coarse stage/detail feedback; it deliberately exposes no percentage, cancel operation, crash journal, or autosave contract.
- Status: Confirmed by code and tests.
...
- Owns: `AppFrameCommandId.SIGNATURE_LIBRARY` and `.SIGN_AND_SAVE`, their typed Signing-menu
  definitions and accessible names, AppFrame routing to the modeless Signature Library and
  current-workspace sign submission, and readiness-driven QAction synchronization.
- Does not own: placement-tool commands or direct viewer/widget mutation.
- Key collaborators: `SigningWorkspaceSessionPort.can_submit_sign_request()`,
  `.submit_sign_request()`, `SigningWorkspaceWidget`, `SigningActionCoordinator`, and
  `SigningWorkspaceActionState`/`WorkspaceActionState`.
- Main entry points: `FoliaSealAppFrame.show_reusable_object_library()`,
  `_save_document()`, `_workspace_ready_to_sign()`, and `_sync_signing_command_action()`.
- Known constraints: Signature Library is always routed through the AppFrame-owned modeless
  ...
  call `_sync_signing_command_action()` so the menu remains truthful. The command registry
  intentionally omits Place, Adjust, and Remove; those controls remain contextual workspace
  interactions rather than top-level Signing commands.
- Status: Implemented and confirmed by focused tests.
