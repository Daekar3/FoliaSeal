...
| Profile terminology remains as compatibility API around signature preset storage and shell methods. | New code may accidentally use old method names and blur the distinction between `SignaturePreset`, `AppearanceProfile`, and `PlacementProfile`. | Current shell/tests use preset-oriented names; old profile methods delegate to canonical preset methods. | Remove compatibility wrappers after remaining external callers are audited. |
