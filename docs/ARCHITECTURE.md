...
- Known constraints: This is still a narrow tracer-bullet seam, not the full harness resolution. The live adapter now translates through the dedicated `testing_adapter` first, then `compat_surface`, then the shell itself for compatibility, while the lower-level preview-analysis payload builder still lives in `phase3_harness.py`. The remaining shell-anatomy knowledge is concentrated in one module instead of being duplicated across preview-matrix, signed-acceptance, and session-runner call sites.
- Status: Confirmed by code and tests.
...
1. `build_qt_app_frame_host()` constructs a `QtAppFrameAdapter` and returns the real `FoliaSealAppFrame` host, while `build_qt_app_frame()` remains the raw-window compatibility projection.
