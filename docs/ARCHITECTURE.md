...
- Owns: `DocumentReviewWorkspaceSession`, `DocumentReviewCardState`, `DocumentTextWorkspaceState`, `DocumentReviewWorkspaceState`, `DocumentReviewWorkspaceTransition`, `DocumentReviewWorkspaceViewerEffects`, selected-signature preservation across review refreshes, search current-hit page-jump intents, independent strong-current/quiet-same-page search highlight effects, text-selection mode transitions, selected-text highlight intents, and restoring the active search summary when text-selection mode is turned off.
