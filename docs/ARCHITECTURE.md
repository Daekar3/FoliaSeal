...
- Known constraints: Typed refs carry stable IDs and known kinds; display names are never parsed into identity. `ReusableCatalogSnapshot` exposes immutable views and typed indexes, not persistence catalogs, stores, or paths. `AppearanceProfile`, `PlacementProfile`, and `SignaturePreset` own persistent `pinned` state. Presets remain reference-only, may omit a certificate reference, and overwrite preserves existing component IDs. Duplicate/reference validation, case-insensitive name uniqueness, composition sequencing, and repository writes happen inside the reusable-object boundary; `DuplicateObject` creates a new unpinned identity and `SetPinned` changes pin state without changing identity or name. A failed write leaves the prior snapshot intact. Production AppFrame composition injects a certificate-configuration existence resolver so preset writes cannot persist dangling certificate references; direct test-only service construction may omit that resolver. Deleting a referenced appearance or placement is rejected; deleting a preset never cascades. The library's create/edit buttons call back into the active signing workflow, where the refinement dialog remains the rich editor; the library does not duplicate those controls. Partial presets render explicit certificate guidance in `SignaturePropertiesCoordinator` before signing. The production shell and coordinator receive one required `ReusableSigningObjects` instance from the AppFrame/workspace composition root; they no longer accept catalog-shaped kwargs or construct a fallback service. Persistence-store parameters remain at AppFrame as the sole filesystem composition edge.
- Status: Confirmed by code and tests.
...
- Known constraints: `FoliaSealAppFrame` owns one reusable-library dialog instance and opens it modelessly; `ReusableObjectLibraryDialog` builds one horizontal three-column `QSplitter` with catalog navigation, a Presets-first/searchable master list, and a details column. `SignatureLibrarySession` owns selection/search projection and the transactional name draft; the dialog delegates reusable-object writes to `ReusableSigningObjects`, including typed duplicate and pin commands, and its explicit `save_detail()` boundary commits the current name transaction before refreshing. AppFrame routes typed certificate pin, rename, and delete callbacks to certificate-catalog policy/manager services, so configured and unconfigured certificate rows are actionable. AppFrame persists the selected catalog and Name A-Z/Z-A order through `AppSettings.ui.library_last_catalog` and `library_sort`. Appearance and preset create/edit remain callbacks into the active signing workflow, while placement create/edit uses the document-independent numeric editor. Certificate create/import/configure operations remain in Settings certificate surfaces; nested document-independent rich preset editors, reason/location defaults, expiration sorting, dirty-detail prompts, and active-placement invalidation prompts remain explicit deferred boundaries. The current Save/Cancel name boundary does not mutate a live signing draft.
- Status: Confirmed by code and tests.
