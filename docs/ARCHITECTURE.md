...
| Keep visible-signature layout policy behind explicit layout boundaries. | `visible_signature_layout.py` exposes `VisibleSignaturePlanner`, the neutral `VisibleSignaturePlan`/`SignatureLayoutPlan` IR, and explicit adapter operations; `presentation/qt/signature_preview_layout.py` turns that into widget-facing preview layout state. Backend, canonical preview, Qt preview, and harness diagnostics consume the same neutral policy/evidence contract; Qt may use presentation-specific measurement settings. The public `SignatureLayoutPlan.backend_reservation` payload is gone; backend compatibility is now carried by evidence names such as `backend_reservation_snapshot` and `backend_reservation_error`. | Confirmed by code/debt |
...
| `src/foliaseal/application/phase3_evidence_service.py` | Explicit injected service boundary for Phase 3 evidence capture, matrix execution, validation, and signed-acceptance evidence generation. | Owns request/result dataclasses and compatibility service verbs consumed by the gateway and legacy adapters. |
...
| `src/foliaseal/application/phase3_evidence_gateway.py` | Reusable caller-facing gateway and document-bound session facade for Phase 3 evidence operations. | `Phase3EvidenceGateway` tags/dispatches effectful operations and validation; `Phase3EvidenceSession` binds PDF credentials and artifact defaults for repeated calls. |
...
| `src/foliaseal/presentation/qt/` | Qt viewer/signing widgets and manual/automated harnesses. | Uses dynamic PySide6 imports so tests can use fakes; the app frame now delegates workspace-open composition to `app_frame_workspace_open.py`, certificate dialog construction/execution to `app_frame_certificate_management.py`, the signing shell owns the workspace bootstrap/port/factory seam in `signing_shell_port.py` where `QtSigningWorkspaceFactory` now returns a `SigningWorkspaceBundle` containing the narrow production `SigningWorkspacePort` plus the explicit `SigningWorkspaceTestingPort`, delegates the extracted `SignaturePropertiesPanel` module to `signing_workspace_properties_panel.py`, the narrow production shell port to `signing_workspace_shell_surface.py`, the neutral testing contracts to `signing_workspace_testing_port.py`, the thin widget-export and harness/testing seam to `signing_workspace_compatibility_surface.py` where `SigningWorkspaceCompatibilitySurface` installs the explicit `SigningWorkspaceTestingPort` implementation as `testing_adapter` and narrows panel access behind a typed `panel` port, the startup/bootstrap ordering and ordered interaction-plan execution to `signing_workspace_orchestrator.py`, and the live review/text/page/rect/current-request verbs to `signing_workspace_runtime.py`, while certificate/preset reconciliation stays in the application coordinator, signing-action orchestration stays in `signing_workspace_action_bridge.py` plus the narrower `signing_action_boundary.py`, canonical preview lifecycle stays in `signature_preview_lifecycle.py`, preview geometry/layout handoff stays in `signature_preview_layout.py`, and the Phase 3 harness stays split across the Qt-backed adapter/composition root in `phase3_harness.py` where `Phase3Harness` consumes the application-layer Phase 3 request dataclasses and exposes explicit `capture()`, `preview_matrix()`, and `signed_acceptance_matrix()` verbs backed by typed internal ports. `phase3_harness.py` is no longer the caller-facing contract; the `run_*` methods are compatibility wrappers rather than stable public seams. `phase3_harness_workspace.py` remains the scenario-application boundary, `phase3_harness_session_runner.py` the Qt session runner, `phase3_preview_matrix_runner.py` the preview-matrix runner, `phase3_signed_acceptance_matrix_runner.py` the signed-acceptance matrix runner, `phase3_signed_acceptance_scenario_executor.py` the signed-acceptance scenario executor, `phase3_appearance_snapshotter.py` the shared appearance snapshotter, `phase3_sign_time_diagnostics_snapshotter.py` the sign-time diagnostics snapshotter, `phase3_image_comparison_helper.py` the shared image-comparison helper, `phase3_text_geometry_helper.py` the shared text-geometry helper, `phase3_signed_output_snapshotter.py` the shared signed-output snapshotter, `phase3_signed_output_render_snapshotter.py` the signed-output render snapshotter, `phase3_harness_capture_assembler.py` the capture assembler, and `phase3_harness_reporting.py` the pure reporting boundary. `phase3_signed_acceptance_evidence.py` is now an internal helper around the application-layer evidence service that only provides default service wiring and runtime-noise suppression, and the harness workspace now reaches the live shell only through `testing_adapter`. |
...
- Known constraints: This module is currently large and contains both concrete adapter logic and many private layout helpers. Recent layout work introduced `VisibleSignatureLayoutEngine`, and visible text/metadata now comes from `VisibleSignatureSemanticsService`. `prepare_phase3_signing_plan()` is the application-owned preparation boundary: it resolves visible semantics and one neutral layout plan, converts layout fit issues to `SigningDraftValidationIssue`, and keeps third-party objects out of the DTO. The signer materializes PyHanko/Pillow objects, constructs hidden fields for invisible headless requests, performs signing/TSA work, and verifies output. `PyHankoSignatureTextBoxEngine` is the concrete owner of PyHanko text-box style and metric preparation; backend compact-reservation checks consume one `PreparedTextBox` from this engine. Background-layout construction for pyHanko stamp styles now routes through `visible_signature_layout.py`, and reservation sizing plus the moved fit-policy helper implementations now live there as well. This module keeps compatibility wrapper names, `build_backend_reservation_evidence()`, and backend-owned fit-issue orchestration/rendered-fit fallback, but not ownership of those core helper implementations. New production callers should not import those backend-private helpers directly. `_build_stamp_text()` remains as a private compatibility wrapper for backend tests and backend-owned evidence assembly. `build_backend_reservation_evidence()` is the boundary for JSON-ready reservation snapshot/error facts; temporary UI layers such as the Phase 3 harness should consume that boundary instead of reconstructing reservation evidence from backend-private helpers. For incremental approval signing, `PyHankoPdfSigner` allocates the first unused `SignatureN` field in the input PDF so subsequent signatures append without modifying the prior signed revision.
...
- Responsibility: Provide the neutral visible-signature planner plus the PyHanko adapter layer that consumes its plan.
- Owns: `VisibleSignaturePlanner`, `VisibleSignatureLayoutBoundary`, `VisibleSignaturePlanRequest`, `VisibleSignaturePlan`, `VisibleSignatureLayoutService`, `VisibleSignatureLayoutInput`, `VisibleSignatureLayoutOptions`, `LayoutRequest`, `SignatureLayoutPlan`, typed text/image/ink metrics, layout fit issues, `TextMeasurer`/`PyHankoTextMeasurer` compatibility ports, pyHanko style facade results, and adapter ports for text measurement, image probing, and horizontal rendered-ink measurement.
...
- Main entry points: `VisibleSignaturePlanner.plan()`, `VisibleSignaturePlanner.prepare_signing_style()`, `VisibleSignaturePlanner.prepare_preview_style()`, `VisibleSignatureLayoutBoundary.plan()`, `VisibleSignatureLayoutService.plan()`, `VisibleSignatureLayoutService.pyhanko_style_for_signing()`, `VisibleSignatureLayoutService.pyhanko_style_for_canonical_preview()`, `VisibleSignatureLayoutEngine.plan()`, `VisibleSignatureLayoutEngine.validate()`, `PyHankoSignatureAppearanceAdapter.build_stamp_style()`, and the public inset operations `single_line_stamp_content_inset()`, `single_line_vertical_stamp_border_gap()`, and `single_line_horizontal_stamp_vertical_inset()`.
...
- Known constraints: The neutral boundary returns `VisibleSignaturePlan` with JSON-ready reservation evidence, while `VisibleSignatureLayoutService` remains the PyHanko/Pillow adapter composition root and accepts an optional precomputed `SignatureLayoutPlan` so signing and preview callers do not silently recompute policy. `SignatureTextBoxEngine` and `PreparedTextBox` live in `signature_text_measurement.py` and expose only application data plus an opaque style token; the production PyHanko implementation remains in `phase3_signing_backend.py`. `PyHankoTextMeasurer` delegates to an injected engine and lazily constructs the production adapter by default, keeping runtime imports one-way at the cost of a deliberate localized cycle risk. `VisibleSignaturePlanner` is the normal application entry point: its plan is neutral, while `prepare_signing_style()` and `prepare_preview_style()` are the explicit materialization operations. `phase3_signing_backend.py`, `signing_preview_renderer.py`, and Qt preview geometry use public operations; backend-private image/fit helpers and Qt inset aliases remain compatibility wrappers for older tests/callers. A multi-provider measurement registry is deferred until provider diversity requires it. The boundary coverage lives in `tests/unit/test_visible_signature_layout_boundary.py` alongside the existing layout tests.
- Status: Confirmed by code and tests.
...
- Responsibility: Own the injected application-layer boundary used by the Phase 3 gateway for harness capture, preview matrices, signed-acceptance matrices, capture validation, and signed-acceptance evidence generation.
...
- Main entry points: `build_default_phase3_evidence_service()`, `Phase3EvidenceService.for_pdf()`, `Phase3EvidenceService.capture_harness()`, `Phase3EvidenceService.run_preview_matrix()`, `Phase3EvidenceService.run_signed_acceptance_matrix()`, `Phase3EvidenceService.validate_harness_capture()`, `Phase3EvidenceService.run_signed_acceptance_evidence()`.
- Known constraints: The service is intentionally thin over injected runners and writers, but it centralizes the request/result types and typed matrix normalization used by the gateway and compatibility adapters. `run_*_matrix()` remain raw-dictionary compatibility methods; typed result methods preserve the raw summary mapping while exposing normalized pass/path/count/error fields. Signed normalization treats nonzero `error_scenario_count` as a failure, and aggregate rows preserve a runner-provided `summary_json_path` with the legacy artifacts-directory fallback. The Qt helper module only suppresses known benign Qt/pyHanko chatter and preserves the documented output paths for default service construction.
...
### Phase 3 evidence gateway and session

- Location: `src/foliaseal/application/phase3_evidence_gateway.py`
- Responsibility: Provide a small reusable caller-facing facade without exposing Qt harness runners or service wiring details.
- Owns: `Phase3EvidenceGateway.run()` for tagged capture, preview-matrix, signed-acceptance-matrix, and signed-acceptance-evidence operations; `Phase3EvidenceGateway.validate()` for read-only capture validation; `Phase3EvidenceGateway.for_pdf()`; `Phase3EvidenceSession.preview()`, `signed_acceptance()`, `capture()`, and `validate()`; `Phase3OperationKind`, `Phase3OperationRequest`, `Phase3ValidationRequest`, and default artifact/checklist paths.
- Does not own: matrix execution, Qt lifecycle, JSON schema changes, or artifact serialization.
- Key collaborators: `Phase3EvidenceServicePort`, `phase3_evidence_service.py`, `qa_evidence_contract.py`, and the existing Phase 3 harness/matrix adapters.
- Main entry points: `gateway_for_service()`, `Phase3EvidenceGateway.run()`, `Phase3EvidenceGateway.validate()`, `Phase3EvidenceGateway.for_pdf()`, and `Phase3EvidenceSession` convenience methods.
- Known constraints: `for_pdf()` binds the PDF, certificate path, passphrase, and default `artifacts/phase3` directory once; per-call artifact overrides remain available. Existing `Phase3EvidenceService`, `Phase3Harness`, and CLI entry points remain compatibility adapters and must preserve their request shapes, raw summary mappings, command names, printed labels, exit behavior, and serialized artifact fields.
- Status: Confirmed by code and tests.
...
- Key collaborators: `Phase3EvidenceService`, `Phase3EvidenceGateway`, `Phase3EvidenceSession`, CLI composition in `__main__.py`, and the existing preview/headless, signed/Qt, and interactive adapters.
...
- Main entry points: `Phase3PreviewMatrixRunner.run()`, `phase3_harness.py::Phase3Harness.preview_matrix()`, `phase3_harness.py::Phase3Harness.run_preview_matrix()`.
...
| `VisibleSignaturePlan` / `VisibleSignaturePlanner` | `application/visible_signature_layout.py` | Neutral immutable visible-signature IR and its application-owned facade. | resolved `SignatureLayoutPlan`, request-derived evidence, typed fit diagnostics; planner exposes `plan()`, `prepare_signing_style()`, and `prepare_preview_style()`. | The plan contains application/domain data only; PyHanko/Pillow objects are created only by explicit adapter preparation. |
...
| `Phase3OperationKind` / `Phase3OperationRequest` / `Phase3ValidationRequest` | `application/phase3_evidence_orchestrator.py` (re-exported by gateway) | Tagged application inputs for effectful operations and read-only validation. | operation kind/payload or summary JSON path. | The orchestrator validates payload types and keeps dispatch explicit; gateway imports remain compatible. |
...
| `Phase3EvidenceGateway` / `Phase3EvidenceSession` | `application/phase3_evidence_gateway.py` | Compatibility gateway and PDF-bound convenience facade. | injected service port; PDF/certificate/passphrase; default artifact directory. | Session methods delegate through the orchestrator and preserve typed matrix results and existing capture/validation outputs. |
...
### Visible-signature planner and adapter contract

- Producer: `VisibleSignaturePlanner.plan()` and its `prepare_*_style()` methods.
- Consumer: `phase3_signing_backend.py`, canonical preview rendering, Qt preview geometry, and tests.
- Stability: Active application boundary; additive facade over the existing service.
- Backward compatibility requirements: A supplied `SignatureLayoutPlan` must be consumed without a second planning pass; preview/signing must retain matching geometry evidence, fit issue codes/messages, and image behavior. Existing underscored backend helpers remain delegating compatibility wrappers until their direct tests are retired.
- Validation: `tests/unit/test_visible_signature_layout_boundary.py` proves shared neutral plans and adapter reuse; focused layout, preview, workflow, Qt, and backend suites preserve numerical and message parity.
...
### Phase 3 evidence gateway/session contract

- Producer: `Phase3EvidenceGateway` and `Phase3EvidenceService.for_pdf()`.
...
- Validation: Gateway delegates dispatch and payload checks to `Phase3EvidenceOrchestrator`; validation delegates to `evaluate_phase3_evidence_contract()` through the service.
...
- Compatibility rule: `Phase3Harness`, `Phase3EvidenceService` raw `run_*` methods, and CLI commands remain compatibility adapters. Preserve their request shapes, command names, printed labels, exit behavior, raw summary mappings, and serialized artifact fields while routing new reusable callers through the gateway/session facade.
- Source files: `src/foliaseal/application/phase3_evidence_gateway.py`, `src/foliaseal/application/phase3_evidence_service.py`, `src/foliaseal/presentation/qt/phase3_harness.py`, `src/foliaseal/__main__.py`.

### Phase 3 evidence orchestrator contract

- Producer: `src/foliaseal/application/phase3_evidence_orchestrator.py`
- Consumer: CLI composition, reusable gateway/session callers, and focused boundary tests.
- Stability: Active application boundary; additive to existing service, harness, and CLI contracts.
...
1. A caller constructs or receives `Phase3EvidenceService`, then wraps it with `gateway_for_service(...)` (or uses the CLI composition helper) to obtain the application boundary.
2. The gateway/session builds tagged `Phase3OperationRequest` values; `Phase3EvidenceOrchestrator.run()` validates each payload and delegates to the injected service port.
...
5. Existing `Phase3Harness`, raw service `run_*` methods, gateway aliases, and serialized outputs remain compatibility adapters for legacy callers.
...
|---|---|---|---|
| The Phase 3 harness still spans several broad presentation helpers even after the gateway/dependency-bundle refactor. | Understanding or changing a full harness run still requires bouncing between the gateway, session runner, matrix runners, scenario executors, and the live testing seam. | `phase3_harness.py` now keeps the explicit three-verb gateway and typed internal ports, `phase3_harness_workspace.py` owns the duplicated live/headless scenario mutation path, and `phase3_harness_session_runner.py` still owns interactive session lifecycle and callback wiring. | Continue deepening the harness in narrow slices and trim the remaining compatibility wrappers once no legacy callers need them. |
...
| Gateway and service compatibility aliases remain during the boundary migration. | Older callers and tests import gateway names and use `for_pdf()`/typed service methods. | Gateway/session stay thin facades and re-export the moved request vocabulary. | Retire aliases only through an explicit migration with compatibility coverage. |
| `phase3_signing_backend.py` still mixes concrete pyHanko adapter code with compatibility-layer visible-signature layout helpers. | Harder to navigate and test at a single public boundary. | Structural reservation sizing and the moved fit-policy helper implementations now live in `visible_signature_layout.py`, while the backend retains compatibility wrapper names, `build_backend_reservation_evidence()`, and backend-owned fit-issue orchestration plus rendered-fit fallback. | Finish moving the remaining fit/rejection helpers behind the layout boundary, then reduce private-helper test reliance after coverage is equivalent. |
| Public planner/adapters coexist with backend compatibility wrappers. | New callers have a stable neutral-plan/materialization boundary, but the backend still exposes private aliases and concrete helper concentration for legacy tests. | Production consumers use `VisibleSignaturePlanner`, public image/fit operations, and public Qt inset operations; underscored names delegate to those operations. | Migrate remaining direct private-helper tests/callers to boundary coverage, then retire wrappers in a separately validated slice. |
...
| Date | Change | Reason |
|---|---|---|
| 2026-07-31 | Added the visible-signature planner/IR hybrid boundary. | Documented `VisibleSignaturePlanner`, neutral `VisibleSignaturePlan`/`SignatureLayoutPlan` ownership, explicit signing/preview materialization, public image/fit/inset operations, compatibility wrappers, and the remaining adapter debt. |
...
| 2026-08-01 | Added the application-owned Phase 3 evidence orchestrator boundary. | Tagged dispatch and capture validation now have one Qt-free application owner; the gateway/session remain compatibility facades, CLI routing crosses the boundary, and preview/headless, signed/Qt, and interactive adapters retain their existing lifecycle and artifact contracts. |
...
| 2026-06-27 | Added the first `Phase3Harness` facade tracer bullet for preview- and signed-acceptance matrix runs. | Reconciled the architecture doc so `phase3_harness.py` briefly exposed an interim deep-module seam before the later hybrid collapse moved the caller-facing contract into `Phase3EvidenceService`. |
| 2026-07-04 | Collapsed the Phase 3 hybrid contract to the application service boundary. | Reconciled the architecture doc so `Phase3EvidenceService` is the only caller-facing Phase 3 contract, `Phase3Harness` is a Qt-backed adapter/composition root that consumes application-layer request dataclasses, and `phase3_signed_acceptance_evidence.py` is only default wiring plus runtime-noise suppression. |
| 2026-07-07 | Rebuilt the Phase 3 harness internals around typed gateway ports and dependency bundles. | Reconciled the architecture doc so `Phase3Harness` keeps explicit `capture()`, `preview_matrix()`, and `signed_acceptance_matrix()` verbs, `Phase3HarnessDependencies` now holds ports instead of runner-builder callables, and the session runner plus matrix helpers now consume typed dependency bundles while preserving the evidence payload contract. |
