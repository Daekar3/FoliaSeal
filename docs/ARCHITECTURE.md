...
| `src/foliaseal/presentation/qt/signing_workspace_compatibility_surface.py` | Thin widget-export and testing seam for the signing workspace shell. | Owns the named `widget.compat_surface`, the explicit `SigningWorkspaceTestingPort` implementation exported as `testing_adapter`, and the concrete widget exports needed because `build_qt_signing_shell()` returns the widget itself rather than the wrapper object. |
...
- Known constraints: The current implementation now hosts pyHanko background-layout construction behind `VisibleSignatureLayoutService` and `PyHankoSignatureAppearanceAdapter`, and reservation sizing plus the moved fit-policy helper implementations now live in `visible_signature_layout.py`. The backend keeps compatibility wrapper names, `build_backend_reservation_evidence()`, and backend-owned fit-issue orchestration/rendered-fit fallback, but the public `SignatureLayoutPlan.backend_reservation` payload has been removed; backend parity now travels through evidence names such as `backend_reservation_snapshot` and `backend_reservation_error`. Production callers should prefer `VisibleSignatureLayoutService` for new signing/preview integration and use `VisibleSignatureLayoutEngine`, `LayoutRequest`, `SignatureLayoutPlan`, and adapter APIs for compatibility during migration. Direct backend-private helper use is limited to backend compatibility wrappers, backend-specific tests, adapter parity tests, and pyHanko-rendered evidence. Remaining cleanup is now about trimming any residual backend helper ownership, not deleting the public layout payload.
- Status: Confirmed by code and tests.
...
- Owns: Evidence contract evaluation, harness capture JSON shape, preview/signed matrix summary generation, Phase 3 evidence orchestration, scoped filtering of known benign evidence-command runtime chatter, checklist rendering. The evidence service owns the explicit caller-facing verbs for capture harness, matrix execution, validation, and signed acceptance evidence; `phase3_harness.py` is the Qt-backed adapter/composition root that consumes service-owned request types, and `phase3_signed_acceptance_evidence.py` only supplies default service wiring and runtime-noise suppression.
...
- Main entry points: `Phase3HarnessSessionRunner.run()`, `phase3_harness.py::_run_phase3_harness_session()`, `phase3_harness.py::Phase3Harness.run_signing_harness()`.
...
- Known constraints: This is still a narrow tracer-bullet seam, not the full harness resolution. The live adapter now translates directly through the explicit `SigningWorkspaceTestingPort` installed as `testing_adapter`, while the lower-level preview-analysis payload builder still lives in `phase3_harness.py`. The remaining shell-anatomy knowledge is concentrated in one module instead of being duplicated across preview-matrix, signed-acceptance, and session-runner call sites.
- Status: Confirmed by code and tests.
...
- Main entry points: `Phase3PreviewMatrixRunner.run()`, `phase3_harness.py::Phase3Harness.run_preview_matrix()`.
- Known constraints: The runner is intentionally headless and keeps `SignaturePresetCatalogStore.default()` inside the helper because the preview-only sweep still needs repository-default preset resolution. `phase3_harness.py` remains the composition root that wires the real manifest loader, scenario executor, error mapper, and JSON normalization helper into the runner.
- Status: Confirmed by code and tests.
...
- Main entry points: `Phase3SignedAcceptanceMatrixRunner.run()`, `phase3_harness.py::Phase3Harness.run_signed_acceptance_matrix()`.
...
| `Phase3HarnessDependencies` / `Phase3Harness` | `presentation/qt/phase3_harness.py` | Qt-backed adapter/composition root for Phase 3 harness modes. | injectable runner builders plus adapter methods that consume application-layer requests. | Delegates to extracted runners; it no longer owns a second public request type. |
...
|---|---|---|---|
| The Phase 3 harness still spans several broad presentation helpers even after the scenario-application extraction. | Understanding or changing a full harness run still requires bouncing between top-level entrypoints, the session runner, matrix runners, scenario executors, and the live testing seam. | `phase3_harness_workspace.py` now owns the duplicated live/headless scenario mutation path as a first tracer bullet, and its live path now depends on `testing_adapter`, while `phase3_harness_session_runner.py` still owns interactive session lifecycle and callback wiring. | Continue deepening the harness in narrow slices and keep trimming leftover compatibility exports once no legacy callers need them. |
...
| Date | Change | Reason |
|---|---|---|
| 2026-07-05 | Deepened the signing-workspace runtime/controller and thinned the compatibility surface. | Reconciled the architecture doc so `signing_workspace_runtime.py` now owns the live review/text/page/rect/current-request verbs while `signing_workspace_compatibility_surface.py` keeps widget export installation and `testing_adapter`. |
