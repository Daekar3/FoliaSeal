...
| `src/foliaseal/presentation/qt/signing_workspace_testing_port.py` | Neutral testing-port contracts for live signing workspace harness callers. | Defines the explicit `SigningWorkspaceTestingPort` plus its narrower `panel` port so production callers can depend on `signing_shell_port.py` without importing the compatibility-surface implementation module. |
...
| `src/foliaseal/presentation/qt/signing_workspace_composition.py` | Extracted workspace-composition helper for the signing workspace. | Owns constructor-time session/bridge/widget assembly and construction of the shell-local runtime, surfaces, and orchestrator while `signing_shell.py` remains the outer shell adapter and public workspace class. |
...
4. The workspace shows the document viewer on the left with a compact page-navigation toolbar above it, plus read-only signing-flow and document-review cards in the right sidebar derived from current draft/readiness/result state and the currently open PDF; the toolbar owns visible previous/next page controls, editable current-page entry, and total-pages readout, while the review card owns only the summary, compact per-signature list, and selector-driven per-signature detail. Reopening the last successful signed output now lives exclusively in the primary sign panel.
