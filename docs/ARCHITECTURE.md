...
| `src/foliaseal/presentation/qt/app_frame_command_model.py` | Typed registry of top-level File, View, and Settings commands. | `AppFrameCommandId` and immutable `AppFrameCommandDefinition` records provide stable IDs, menu text, shortcuts, unique mnemonic labels, and accessible names; `FILE_COMMAND_DEFINITIONS`, `VIEW_COMMAND_DEFINITIONS`, `SETTINGS_COMMAND_DEFINITIONS`, and `ALL_COMMAND_DEFINITIONS` form one registry, and the frame maps these definitions to its owned Qt actions. Unsupported Edit, Signing, Help, and remaining View commands are intentionally absent until truthful behavior seams exist. |
...
- Known constraints: Widgets use dynamic PySide6 imports and many test doubles. `app_frame.py` owns the top-level `QMainWindow`, menu routing, frame-owned QAction mutation, the no-document landing panel, the reusable-object Library window, and Qt bootstrap helper `launch_qt_app_frame()`. The typed `app_frame_command_model.py` registry supplies stable File-, View-, and Settings-command IDs plus Qt-supported shortcut, unique mnemonic, object-name, tooltip/status metadata; the frame keeps the resulting command-to-action mapping and applies the definitions' Qt metadata at that edge. File command callbacks route through the active workspace's maintenance port for output-path selection/presence and through its session port for signing submission; View Previous Page and Next Page callbacks route through that same public session port, while `has_explicit_output_pdf_path()` remains the explicit output-path seam. The five Settings definitions map to concrete frame callbacks: `APPLICATION_SETTINGS` → `show_app_settings()`, `MANAGE_REUSABLE_OBJECTS` → `show_reusable_object_library()`, `CREATE_CERTIFICATE` → `show_certificate_creation()`, `IMPORT_CERTIFICATE` → `show_certificate_import()`, and `MANAGE_CERTIFICATE_CONFIGURATIONS` → `show_certificate_management()`. The no-document panel keeps the frame mounted before any PDF is opened and exposes direct `Open a PDF…` and `Manage Signature Library…` buttons; it must not restore a document, draft, or recent-file state. The Library entry point is modeless and AppFrame-owned: `show_reusable_object_library()` creates at most one `ReusableObjectLibraryDialog`, refreshes an existing instance, and raises/activates it without blocking the main frame. The Qt-free `app_frame_workspace_action_state.py` projection owns the action-state policy, including page-navigation capability flags; the frame applies its enabled/checked fields at the QAction edge and resynchronizes navigation after opening and navigation commands. `FoliaSealAppFrame` owns `restore_window_geometry()` before display, `capture_window_geometry()` after the event loop, and `persist_captured_window_geometry()` through `AppSettingsStore`; restore clamps saved positions to the available screen and capture enforces the typed 1100x700 minimum. Full rail-divider persistence, Library geometry/minimums, monitor/DPI-aware rerendering, and toolbar overflow/persistence remain deferred. `app_frame_workspace_open.py` loads the page count, builds the viewer/signing workflows, and returns a typed `WorkspaceHandle`. `SigningWorkspaceHost` is the sole active-workspace source: `current_workspace`, `current_shell`, `current_viewer_workflow`, and `current_signing_workflow` are read-only projections of `host.active()`, with no duplicated `_current_workspace`, shell-port, or compatibility snapshot state. `QtWorkspaceMount` owns `QMainWindow.setCentralWidget(...)` during replacement; the frame does not install replacement widgets directly. `SigningWorkspaceWidget` is the explicit composite facade and declares the production viewer/navigation verbs (`refresh_viewer()`, `go_to_previous_page()`, `go_to_next_page()`, `can_go_previous_page()`, `can_go_next_page()`, `reset_zoom_view()`, `setFocus()`) used by its typed adapters. `QtSigningWorkspaceFactory.create()` is the sole production composition caller and delegates one unchanged `SigningWorkspaceBootstrap` to `SigningShellAdapter`; the typed `QtSigningWorkspaceCompositionRequest` hides widget/callback choreography behind a semantic host-actions record. `signing_shell_port.py` provides the maintenance, primary-flow session, opaque view, and testing capabilities grouped in `SigningWorkspaceBundle`; the bundle is the sole harness contract and no caller reaches through a compatibility surface, widget exporter, or raw shell fallback. The legacy `build_qt_signing_shell()` function remains only as a test adapter until direct shell-test callers are migrated. Stable Phase 3 CLI commands, manifest keys, JSON fields, DTO names, and artifact paths remain external compatibility contracts.
...
- Owns: `SigningWorkspaceBundle.maintenance` for settings/certificate/profile/text maintenance, `.session` for review/place/preview/sign/navigation sequencing (including declared previous/next-page and zoom-reset methods), `.testing` for non-production diagnostics and harness reads, and `.view` for opaque mount/dispose lifecycle operations. `WorkspaceHandle` republishes the same typed capabilities together with the document-bound workflows after successful composition.
