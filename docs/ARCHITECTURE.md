...
- Key collaborators: `phase3_harness.py`, `phase3_signed_output_snapshotter.py`, `_render_signed_annotation_appearance_direct()`, `_preview_appearance_snapshot_from_capture()`, `_signed_output_appearance_snapshot()`, `compare_signature_appearance_snapshots()`.
- Main entry points: `Phase3SignedOutputRenderSnapshotter.run()`, `phase3_harness.py::_snapshot_signed_output_render()`.
- Known constraints: The render snapshotter intentionally remains in the Qt harness package because it composes many harness-private render and comparison helpers. `phase3_harness.py` remains the composition root that wires those real callables into the helper while the payload contract stays unchanged.
- Status: Confirmed by code and tests.
