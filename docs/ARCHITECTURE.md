...
- Known constraints: The helper must stay failure-tolerant for missing or unreadable PDFs and should only claim that signatures were verified locally, not that they are trusted by any external policy source. Top-level summary text remains latest-signature oriented for compact shell display, while `signature_items` now carries both the compact row text and a selector-driven drill-in detail payload for each embedded signature. Certification guidance is surfaced as plain-language status, not as a write action.
