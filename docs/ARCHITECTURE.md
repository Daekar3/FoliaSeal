...
- Known constraints: Widgets use dynamic PySide6 imports and many test doubles. `app_frame.py` owns a first-pass `QMainWindow` wrapper with File/Open, Settings/Application settings, and Settings/Import certificate actions; the settings action opens an app-wide settings dialog for default directories, and the import action opens a PKCS#12 import dialog. `signing_shell.py` is a large module with control dataclasses, helper functions, properties panel, workspace widget, and shell adapter in one file. The shell can select and apply existing certificate configurations and can refresh its selector after app-frame import, but certificate creation, export/backup, deletion, and secure password storage are still pending. The signing shell consumes `AppSettings` for output-path defaults but no longer edits app-wide directory settings directly.
...
| Managed certificate configuration | Certificate import service, tests, or Qt shell selection | imported PKCS#12 -> managed certificate record + certificate configuration -> resolver -> runtime signing material -> signing draft | XDG data dir under `FoliaSeal/Certificates/certificates.json`; PKCS#12 files under `FoliaSeal/Certificates/Managed/` | `CertificateCatalog` JSON plus PKCS#12 files | JSON stores secret references only; no plain certificate passwords. Qt app frame can import existing PKCS#12 files and Qt shell can select existing configurations; create/export/delete and secure password storage remain pending. |
...
| Full certificate management is not implemented yet. | Users can import existing PKCS#12 files and select resulting certificate configurations, but they cannot yet create, export, back up, delete, or save passwords for managed certificates from the GUI. | Tests and lower-level stores can create catalogs; the app frame owns first-pass import and the shell consumes configurations through the resolver. | Expand the certificate management surface for create/export/delete and add a real credential-store adapter for saved passwords. |
