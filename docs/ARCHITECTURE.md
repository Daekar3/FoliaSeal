...
- Owns: application `AppearanceProfile`, `PlacementProfile`, `SignaturePreset`, `ResolvedSignaturePreset`, `SignaturePresetCatalog`, `ReusableObjectValidationError`, `ReusableSigningObjects`, `CatalogRepository`, `AppSettings`, `TrustProfile`, `TimestampPolicy`, `ManagedCertificate`, `CertificateConfiguration`, `CertificateCatalog`, `CertificateCatalogRepository`, `InMemoryCertificateCatalogRepository`, `CertificateManager`, its typed request/result values, `CertificateSecretStore`/`CertificateSecretStoreError`, `SecretToolCertificateSecretStore`, `SigningMaterial`, `CertificateSigningMaterialResolver`, and infrastructure `AppSettingsStore`, `SignaturePresetCatalogStore`, `certificate_codecs`, and `CertificateCatalogStore`.
...
| Date | Change | Reason |
|---|---|---|
| 2026-08-06 | Moved managed-certificate transaction ownership behind repository verbs. | `CertificateManager` retains certificate policy, PKCS#12/secret compensation, and typed results; `CertificateCatalogRepository` now owns atomic commit/delete operations, `CertificateCatalogStore` owns filesystem/catalog staging and recovery, and the in-memory adapter mirrors bytes for deterministic boundary tests. Broken-symlink/export and missing-file/delete parity are covered. Resolver path properties remain a follow-up, and phase3 nomenclature/contracts remain unchanged. |
