...
- Known constraints: Widgets use dynamic PySide6 imports and many test doubles. `app_frame.py` owns a first-pass `QMainWindow` wrapper with File/Open and Settings menu actions; the Settings action opens an app-wide settings dialog for default directories. `signing_shell.py` is a large module with control dataclasses, helper functions, properties panel, workspace widget, and shell adapter in one file. The shell can select and apply existing certificate configurations, but it does not yet provide certificate create/import/export/delete management. The signing shell still contains duplicate default-directory controls from the pre-app-frame settings slice.
...
- Location: `src/foliaseal/infra/config/schemas.py`, `src/foliaseal/infra/config/app_settings_storage.py`, `src/foliaseal/infra/config/profile_storage.py`, `src/foliaseal/infra/config/certificate_storage.py`, `src/foliaseal/application/signing_material_resolver.py`
...
- Consumer: Qt signing shell settings controls and save-output file-dialog default directory behavior; future application-frame Settings menu and Open-file behavior.
- Stability: Persisted file contract.
...
2. The Settings controls can update default open/output directories and persist them through `AppSettingsStore.save_settings()`.
...
| Default-directory settings controls are duplicated. | Users can edit the same defaults from the app-frame Settings dialog and the signing shell settings group. | Both write the same `AppSettingsStore`; app-frame save propagates settings to the loaded shell. | Remove or demote the signing-shell settings group once the app-frame dialog is the established settings surface. |
