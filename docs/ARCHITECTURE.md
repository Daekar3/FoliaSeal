...
- Known constraints: The shell still owns overwrite confirmation and the boundary still owns error emission routing. The sidebar owns widget mutation for returned state, and the coordinator intentionally returns immutable snapshots so shell and sidebar tests can focus on adapter behavior instead of duplicated state policy. `recommended_action` identifies the currently supported sign, reopen, and preserved-artifact recovery affordances. Recovery deliberately never marks an untrusted artifact as a successful signed output; full asynchronous signing/progress and later-approval permission policy remain deferred until their owning seams exist.
- Status: Confirmed by code and tests.
...
- Known constraints: `apply_signing_action_state()` must remain safe when the container reports zero width during early layout or tests; it falls back to a sensible width limit so the flow-detail label does not collapse. The interactive action container must remain outside the read-only `status_region`, and the protected status region must contain labels only. The sidebar renders the coordinator's typed `recommended_action` with visible and accessible treatment on at most one supported button. Recovery controls never imply that a preserved artifact is trusted; full asynchronous progress, later-approval permission state, independently scrollable upper/lower regions, and a remembered divider topology remain explicitly deferred; the sidebar is a widget-level renderer only while the shell keeps the higher-level orchestration boundary.
- Status: Implemented and confirmed by code and tests.
...
| `SigningActionState` | `presentation/qt/signing_action_coordinator.py` | Immutable Qt-facing projection of the supported signing-action state. | stage/detail/result text, sign/reopen/recovery enablement, last result/output, typed `recommended_action`. | Recovery exposes Verify again, Return to draft, and Open preserved copy without treating the artifact as safe; asynchronous progress and later-approval permission policy remain deferred. |
