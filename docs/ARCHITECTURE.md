...
- Key collaborators: `phase3_harness_capture_assembler.py`, `phase3_harness_reporting.py`, `build_qt_signing_shell()`, `SigningDraftWorkflow`, `ViewerWorkflow`.
- Main entry points: `_run_phase3_harness_session()`, `run_phase3_signing_harness()`.
- Known constraints: The helper keeps the interactive shell flow Qt-bound, but the collected session result must stay stable enough for later payload assembly and report finalization. `run_phase3_signing_harness()` now passes `Phase3HarnessSessionResult` into `Phase3HarnessCaptureAssembler.build_capture_payload()` instead of deriving final capture data inline.
- Status: Confirmed by code and tests.
