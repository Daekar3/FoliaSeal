...
| `src/foliaseal/presentation/qt/app_frame_workspace_action_state.py` | Qt-free app-frame projection of workspace QAction policy. | `WorkspaceActionState` and its pure closed/open/selection-result/native-edit constructors describe whether Save, Save As, Close, Undo, Redo, Cut, Copy, Paste, Select All, Previous Page, Next Page, Back, Forward, text selection, and Copy selected text should be enabled or checked. Undo/Redo fields project the currently selected history owner; native Edit fields project focused-editor capabilities; the projection owns policy only, while `FoliaSealAppFrame` mutates the concrete QActions. |
...
| `src/foliaseal/presentation/qt/app_frame_command_model.py` | Typed registry of top-level File, Edit, View, Signing, and Settings commands. | `AppFrameCommandId` and immutable `AppFrameCommandDefinition` records provide stable IDs, menu text, shortcuts, unique mnemonic labels, and accessible names; `FILE_COMMAND_DEFINITIONS`, `EDIT_COMMAND_DEFINITIONS`, `VIEW_COMMAND_DEFINITIONS`, `SIGNING_COMMAND_DEFINITIONS`, `SETTINGS_COMMAND_DEFINITIONS`, and `ALL_COMMAND_DEFINITIONS` form one registry, and the frame maps these definitions to its owned Qt actions. Edit Undo (`Ctrl+Z`), Redo (`Ctrl+Shift+Z`), Cut (`Ctrl+X`), Copy (`Ctrl+C`), Paste (`Ctrl+V`), and Select All (`Ctrl+A`), View Back (`Alt+Left`), Forward (`Alt+Right`), Select Text, Zoom In/Out (`Ctrl++`/`Ctrl+-`), Reset Zoom, Fit Page (`Ctrl+0`), Fit Width (`Ctrl+Shift+0`), Find (`Ctrl+F`), Signing Signature Library, Place Signature, Adjust Placement, Remove Placement, and Sign and save are truthful actions routed through public workspace ports or native focused-editor behavior; viewer Select All, Help, and remaining unsupported commands remain separate until their behavior seams exist. |
...
- Status: Implemented and confirmed by focused fake-Qt and real offscreen tests; transport limitation
  and compatibility/phase3 retirement remain open.
...
- Does not own: percentage calculation, cancellation, crash journals, autosave, or restart
  restoration.
...
- Known constraints: Begin validates and marks one request active, starts the owned daemon worker,
  ...
  no percent estimate and no cancel action; crash-journal/autosave persistence remains deferred.
- Status: Implemented and confirmed by focused tests.
...
- Known constraints: `apply_signing_action_state()` must remain safe when the container reports zero width during early layout or tests; it falls back to a sensible width limit so the flow-detail label does not collapse. The interactive action container must remain outside the read-only `status_region`, and the protected status region must contain labels only. The sidebar renders the coordinator's typed `recommended_action` with visible and accessible treatment on at most one supported button. Recovery controls never imply that a preserved artifact is trusted; the bounded later-approval permission gate is rendered, and coarse truthful transaction stages are projected while percentage/cancellation behavior remains out of scope. Independently scrollable upper/lower regions and a remembered divider topology remain explicitly deferred; the sidebar is a widget-level renderer only while the shell keeps the higher-level orchestration boundary.
- Status: Implemented and confirmed by code and tests.
...
8. Settings/Application settings opens an editable dialog for default open and output directories, saves through `AppSettingsStore`, and refreshes the frame/current shell settings through the live workspace port.
9. Settings certificate actions route through `app_frame_certificate_management.py`, which constructs and executes the dialogs, delegates typed create, import, rename, delete, and export operations to `CertificateManager`, and then refreshes the loaded signing shell certificate selector through the live workspace port when an operation result reports a catalog change.
10. Open and certificate operation failures are reported through the frame warning/error callback path.
