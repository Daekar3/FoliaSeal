...
| 2026-08-16 | Added the optional X11 AT-SPI audit boundary. | `scripts/x11_atspi_probe.py` is a host-Python, read-only, PID/title-owned probe invoked only by `live_gui_accessibility_audit.py --probe-atspi`; it records accessible-tree data when `org.a11y.atspi.Registry` is present and otherwise returns `unavailable` without changing product dependencies. The current Cinnamon/X11 session lacks that registry service, so no screen-reader or accessible-tree claim is made. |
