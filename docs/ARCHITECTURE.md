...
- Owns: Evidence contract evaluation, harness capture JSON shape, preview/signed matrix summary generation, Phase 3 evidence orchestration, scoped filtering of known benign evidence-command runtime chatter, checklist rendering. `Phase3EvidenceGateway` and its document-bound `Phase3EvidenceSession` are the reusable caller-facing facade; the evidence service remains the injected application boundary underneath. `phase3_harness.py`, the service's raw `run_*` methods, and the CLI dispatch remain compatibility adapters over that boundary, while `phase3_signed_acceptance_evidence.py` only supplies default service wiring and runtime-noise suppression.
...
- Status: Confirmed by code and tests; post-fix validation covers 23 focused Phase 3 evidence/gateway/matrix tests and 1,011 full-suite tests, with Ruff clean.
...
| `Phase3OperationKind` / `Phase3OperationRequest` / `Phase3ValidationRequest` | `application/phase3_evidence_gateway.py` | Tagged gateway inputs for effectful operations and read-only validation. | operation kind/payload or summary JSON path. | Keeps dispatch explicit while reusing service-owned request dataclasses. |
| `Phase3EvidenceGateway` / `Phase3EvidenceSession` | `application/phase3_evidence_gateway.py` | Reusable gateway and PDF-bound convenience facade. | injected service port; PDF/certificate/passphrase; default artifact directory. | Session methods delegate to the gateway and preserve typed matrix results and existing capture/validation outputs. |
...
- Validation: Gateway dispatch checks that each operation kind carries its expected service request type; validation delegates to `evaluate_phase3_evidence_contract()` through the service.
...
1. A caller constructs or receives `Phase3EvidenceService`, then calls `service.for_pdf(...)` (or wraps the service with `gateway_for_service(...)`) to bind one PDF and its certificate credentials.
2. `Phase3EvidenceSession.preview()` and `signed_acceptance()` build service-owned `Phase3MatrixRequest` values and return typed `Phase3MatrixResult` values while preserving existing matrix summary artifacts.
3. `Phase3EvidenceSession.capture()` delegates a `Phase3HarnessCaptureRequest` through the tagged gateway; default checklist paths and `artifacts/phase3` are applied unless overridden.
4. `Phase3EvidenceSession.validate()` delegates a read-only `Phase3ValidationRequest` to the existing evidence-contract evaluator.
5. Existing `Phase3Harness`, raw service `run_*` methods, and CLI handlers remain compatibility adapters for legacy callers and serialized outputs.
