...
| `src/foliaseal/presentation/qt/signing_workspace_compatibility_surface.py` | Broad compatibility surface for the signing workspace shell. | Owns the widget export block, named `widget.compat_surface`, the explicit `SigningWorkspaceTestingPort` implementation exported as `testing_adapter`, and the deep harness/testing helpers that still need access to viewer, panel, review/text, rect/page operations, and live-shell signature-rect priming choreography. |
