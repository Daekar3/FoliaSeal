...
- Backward compatibility requirements: Preserve the visible-signature and placement control structure, `VisibleSignatureSetupDraft` load/build parity, the `Show field names` toggle, per-field visibility checkbox behavior, loaded `field_order` preservation, font-style availability rules, and generic/page-change callback behavior. The advanced per-field override editor and shell-exposed `field_controls` are no longer part of this contract.
