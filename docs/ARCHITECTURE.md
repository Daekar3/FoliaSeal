...
| `Phase3HarnessRequest` / `Phase3HarnessDependencies` / `Phase3Harness` | `presentation/qt/phase3_harness.py` | Caller-facing facade tracer bullet for Phase 3 harness modes. | request-level harness inputs plus injectable runner builders. | First narrow deep-module seam; `run_preview_matrix()` and `run_signed_acceptance_matrix()` are public on the facade while legacy free functions remain compatibility wrappers. |
