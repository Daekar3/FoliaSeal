...
| Keep visible-signature layout policy behind one prepare-once boundary. | `visible_signature_layout.py` exposes the neutral `VisibleSignatureLayoutPort`/`VisibleSignatureLayoutService`; `VisibleSignaturePreparation` freezes the plan, fit decision, and reservation evidence before signing or canonical-preview materialization. `visible_signature_layout_adapters.py` owns Pillow/PyHanko materializers. Qt preview remains a presentation adapter over neutral layout data; preview-only stamp suppression is explicit. | Confirmed by code |
...
| `src/foliaseal/presentation/qt/` | Qt viewer/signing widgets and manual/automated harnesses. | Uses dynamic PySide6 imports so tests can use fakes; app-frame, signing-shell, workspace, lifecycle, reporting, matrix, snapshotter, render, runtime, projection, runner-factory, and artifact responsibilities are documented in their focused modules below. `evidence_harness_runtime.py` owns the typed lazy operation bundle, `evidence_harness_projection.py` owns pure matrix projection, `evidence_interactive_capture.py` owns the stable capture DTO/payload projection/JSON normalization/artifact policy, `evidence_runner_factories.py` owns concrete lazy graph construction, and `phase3_harness.py` remains the concrete Qt composition root. The application service/program remains the caller boundary. |
...
- Owns: application `AppearanceProfile`, `PlacementProfile`, `SignaturePreset`, `ResolvedSignaturePreset`, `SignaturePresetCatalog`, `ReusableObjectValidationError`, `ReusableSigningObjects`, `CatalogRepository`, `AppSettings`, `AppSettingsStore`, `TrustProfile`, `TimestampPolicy`, `ManagedCertificate`, `CertificateConfiguration`, `CertificateCatalog`, `CertificateCatalogStore`, `CertificateManager`, its typed request/result values, `SecretToolCertificateSecretStore`, `SigningMaterial`, `CertificateSigningMaterialResolver`, and infrastructure `SignaturePresetCatalogStore`.
...
| `application` | `domain`, small infra protocols/adapters where currently wired. | Qt presentation widgets; the neutral visible-signature layout module must not import Pillow, PyHanko, Qt, or `phase3_signing_backend`. | `visible_signature_layout_adapters.py` is the explicit concrete Pillow/PyHanko edge; the two Qt `VisibleSignatureLayoutEngine.plan()` callers are geometry-only consumers. Other application modules may still import transitional DTOs/backend helpers; see debt. |
...
| `phase3_signing_backend.py` still mixes concrete pyHanko adapter code with behavior-bearing rendered-ink policy. | The authoritative rendered-ink fallback remains close to signing orchestration, while duplicate fit-gate mutation is centralized. | `VisibleSignaturePreparation` captures the fit decision once and `visible_signature_fit_policy.py` applies it consistently; backend helpers remain concrete measurement policy. | Extract the rendered-ink adapter only when a second concrete consumer justifies it; do not add compatibility facades. |
