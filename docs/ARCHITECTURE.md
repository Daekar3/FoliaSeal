...
| `src/foliaseal/presentation/qt/app_frame_workspace_action_state.py` | Qt-free app-frame projection of workspace QAction policy. | `WorkspaceActionState` and its pure closed/open/selection-result constructors describe whether Save As, text selection, and Copy selected text should be enabled or checked. The projection owns policy only; `FoliaSealAppFrame` remains responsible for mutating the concrete QActions. |
