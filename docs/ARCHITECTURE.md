...
| `src/foliaseal/presentation/qt/app_frame_command_model.py` | Typed registry of top-level File and View commands. | `AppFrameCommandId` and immutable `AppFrameCommandDefinition` records provide stable IDs, menu text, shortcuts, mnemonic labels, and accessible names; `FILE_COMMAND_DEFINITIONS`, `VIEW_COMMAND_DEFINITIONS`, and `ALL_COMMAND_DEFINITIONS` form one registry, and the frame maps these definitions to its owned Qt actions. |
...
2. `FoliaSealAppFrame` creates a `QMainWindow`, mounts the no-document landing panel, and installs the typed File/Open, File/Save, File/Save As, File/Close, File/Exit, View/Previous Page, and View/Next Page actions alongside the Settings actions. The landing panel provides direct `Open a PDF…` and `Manage Signature Library…` actions while the document-dependent actions remain in their closed state, including both View navigation actions.
