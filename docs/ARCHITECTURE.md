...
| `src/foliaseal/presentation/qt/app_frame_profile_library.py` | Qt adapter for the modeless Signature Library. | Builds the AppFrame-owned three-column `QSplitter` (catalog navigation, master/search rows, details), delegates state to `SignatureLibrarySession`, gates destructive mutations behind an explicit Yes/No question, refreshes and reselects a retained certificate after successful configuration, and owns the nested Appearance detail-editor session with suspended parent selection and typed dirty resolution. |
...
- Known constraints: The production editor is a modeless Library detail child and mounts only appearance/content controls inside a scroll area beneath a sticky textual preview labeled as synthetic and never saved. It deliberately does not implement final rendered-preview fidelity, image normalization, reason/location defaults, preset-child return, or active-placement invalidation; those remain explicit follow-up boundaries. The modal dialog is retained only as a compatibility/test wrapper until its direct consumers are retired.
- Status: Confirmed by code and tests.
...
5. Appearance Create/Edit replaces the detail column with `AppearanceProfileEditorWidget`; its Save/Discard/Continue resolver restores the suspended parent selection and name draft, and only Save dispatches `SaveAppearance`. The compatibility `AppearanceProfileEditorDialog` wraps the same widget for direct/test callers, while production AppFrame wiring uses the nested path. Preset Create/Edit remains modal until its nested child plan is implemented.
