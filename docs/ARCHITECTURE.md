...
| `src/foliaseal/application/signing_executor.py` | Neutral lazy production boundary for one signing-request executor. | `LazySigningRequestExecutor` defers construction of the historical concrete backend until the first sign, keeping GUI startup light while exposing one stable `execute` verb; it is the sole new consumer of that historical import and can be retired after backend migration. |
...
| `src/foliaseal/presentation/qt/app_frame_profile_library.py` | Qt adapter for the modeless Signature Library. | Builds the AppFrame-owned three-column `QSplitter` (catalog navigation, master/search rows, details), delegates state to `SignatureLibrarySession`, gates destructive mutations behind an explicit Yes/No question, refreshes and reselects a retained certificate after successful configuration, and routes supported mutations/editors to application services or document-independent editors. |
