...
| `src/foliaseal/presentation/qt/app_frame_profile_library.py` | Qt adapter for the modeless Signature Library. | Builds the AppFrame-owned three-column `QSplitter` (catalog navigation, master/search rows, details) with a vertically scrollable detail surface and a Library-wide footer for nested Appearance editing, delegates state to `SignatureLibrarySession`, owns public layout capture/restore inputs, and leaves atomic settings persistence to the AppFrame. It supports non-persisting first-use Presets focus, gates destructive mutations behind an explicit Yes/No question, notifies the active signing shell after reusable-object saves, refreshes and reselects a retained certificate after successful configuration, and owns the nested Appearance and Signature Preset detail-editor sessions with suspended parent selection and typed dirty resolution. Before opening a nested Preset it refreshes the provider-backed certificate catalog; the Preset editor exposes typed create/import callbacks that return a saved certificate configuration without applying it to the active draft. |
| `src/foliaseal/presentation/qt/appearance_profile_editor_widget.py` | Reusable Appearance detail editor mounted by the modeless Library and compatibility dialog wrapper. | Owns an isolated `SignatureAppearance` draft, compact content controls on the left, a labeled synthetic preview beside and outside the scrolling controls, stable-id-aware Save, a Library-wide action row, and a caller-owned Cancel request; synthetic signer data never enters persistence. |
| `src/foliaseal/presentation/qt/signature_preset_editor_widget.py` | Library-owned nested Signature Preset editor. | Owns the isolated preset draft, stable-id Save, appearance-child entrypoint, explicit create/import Certificate and Placement return actions, provider-backed selector refresh, and child-first dirty resolution; it returns to the Library detail column without requiring an open PDF or mutating the active signing draft. |
...
| `src/foliaseal/presentation/qt/signing_workspace_properties_panel.py` | Extracted Qt properties-panel module for the signing workspace. | Owns `SignaturePropertiesPanel`, its preview/validation widget helpers, setup-session prompt adapter, explicit no-preset guidance, typed Library create/manage action, and panel disposal behavior while `signing_shell.py` remains the composition root. Rail-owned groups and selectors explicitly accept the rail's constrained width; the panel reapplies preview geometry after resize so fixed preview cards cannot impose a horizontal scrollbar or clip helper text. |
...
| `src/foliaseal/presentation/qt/signature_preview_layout.py` | Widget-facing preview geometry planning and application. | Owns preview card sizing, orientation, ordering, and widget visibility decisions. Its applied geometry is recalculated when the properties panel receives its actual rail width, preserving readable preview content at the supported 280–640px rail range. |
| `src/foliaseal/presentation/qt/signature_preview_lifecycle.py` | Canonical preview snapshot lifecycle for the Qt shell. | Owns canonical render invocation, backend reuse, pixmap loading, snapshot replacement, and best-effort cleanup when a replacement pixmap cannot be loaded. |
...
- Owns: `VisibleSignatureLayoutPort`, `VisibleSignatureLayoutRequest`, immutable `VisibleSignaturePreparation`, `VisibleSignatureLayoutService`, stateless `VisibleSignatureLayoutPolicy`, `LayoutSpacing`, public `SignatureLayoutReservation`, `VisibleSignatureLayoutInput`, `VisibleSignatureLayoutOptions`, `SignatureLayoutPlan`, typed text/image/ink metrics, layout fit issues, neutral measurement/materializer ports, target result types, and adapter ports for text measurement, image probing, and horizontal rendered-ink measurement. The former `LayoutRequest` compatibility alias was retired after all in-repository callers migrated to `VisibleSignatureLayoutInput`.
...
- Responsibility: Search the currently opened PDF in a read-only way, track the active query/current hit, and expose plain-language shell state for navigation/highlighting.
...
- Current lifecycle source of truth: `SigningWorkspaceHost` owns the active `WorkspaceHandle`; the frame does not store `_current_workspace`, `_current_shell_port`, or a `WorkspaceCompatibilityState`. Frame `current_*` properties are projections of `host.active()`, and source-recovery acknowledgement now consumes one typed handle through `_with_current_workspace(...)` rather than performing separate compatibility lookups; source-safety refresh crosses `SigningWorkspacePort.refresh_source_safety()` instead of inspecting the opaque view mount target. The host/lifecycle path owns central-widget mounting and `SigningWorkspacePort` has no `widget()` escape hatch. Interactive and signed-acceptance Acceptance runners reuse one `SigningWorkspaceBundle`, whose `.testing` adapter is the sole harness contract. The action-state projection does not add lifecycle or maintenance routing: `SigningWorkspaceHost` remains the sole compose/mount/publish/dispose owner, and text-selection/copy commands continue through the existing maintenance port.
...
- Main entry points: `SigningActionBoundary.load()`, `SigningActionBoundary.accept_output_path()`, `SigningActionBoundary.submit()`, `SigningActionBoundary.begin_transaction()`, `SigningActionBoundary.poll_transaction()`, `SigningActionBoundary.close_transaction()`, `SigningActionBoundary.open_signed_output()`, `SigningActionBoundary.invalidate()`, and the read-only `transaction_active` activity property.
- Known constraints: `begin_transaction()` starts the owned worker only after the coordinator accepts a request; a configured signing executor without an owned transaction runner, or a worker-start exception, is converted into a terminal `sign_failure` transition and routed through visible error/status callbacks rather than leaving the shell at Step 5. `poll_transaction()` is the caller-thread delivery point for worker completion and errors, while `close_transaction()` joins and releases the owned worker. `open_signed_output()` must only forward the callback when the coordinator reports a successful output path, and the boundary must continue to return immutable result snapshots so shell tests can focus on delegation behavior.
- Status: Confirmed by code and tests.
...
- Known constraints: Begin validates and marks one request active, starts the owned daemon worker,
  and returns the active state; a missing runner or worker-start exception is completed as a
  visible `sign_failure` rather than silently leaving the UI at Step 5. Completion is delivered only by boundary polling, then the
...
- Known constraints: `apply_signing_action_state()` must remain safe when the container reports zero width during early layout or tests; it falls back to a sensible width limit so the flow-detail label does not collapse. The interactive action container must remain outside the read-only `status_region`, and the status region must contain labels only. The sidebar renders the coordinator's typed `recommended_action` with visible and accessible treatment on at most one supported button. Recovery controls never imply that a preserved artifact is trusted; the bounded later-approval permission gate is rendered, and coarse truthful transaction stages are projected while percentage/cancellation behavior remains out of scope. The viewer and properties interiors are independent scroll areas. Document-text controls use separate query/navigation rows so labels remain usable at the legal 280-pixel rail minimum; a real-Qt regression assertion guards non-zero child widths there. `SigningWorkspaceComposition` owns the injected `QSplitter`, restores `AppUiSettings.rail_width`, and captures it through `WorkspaceViewPort.capture_ui_settings()`; the app frame persists the resulting mapping atomically. The narrow HBox fallback exists only for incomplete fake binding objects used by legacy unit fixtures and is not the production path.
- Status: Implemented and confirmed by code and tests.
...
- Known constraints: `SigningWorkspaceActionBridge` reads setup labels through the port and never
  reaches `_setup_session`. Refinement observation is test-local: a test-only subclass injects the
  dialog's existing `active_state_changed` callback, while production panel composition does not
  retain a dialog reference or pass an observer. AppFrame dialog-exposure properties and certificate
  dialog compatibility fields remain intentional compatibility surfaces owned by their respective
  frame/certificate plans. The `acceptance` nomenclature plan remains the atomic migration owner for
  historical evidence names.
- Status: Implemented and confirmed by the focused refinement boundary (`5 passed, 105 deselected`)
  and a zero-match grep for `_active_refinement_dialog` and `_set_active_refinement_dialog` under
  `src` and the affected test. Broader full-suite, display-backed, and acceptance release gates are
  tracked by their owning plans.
...
- Known constraints: Cleanup is best-effort and only removes temporary canonical-preview directories; normal widget close must reach `dispose()` so temp snapshots are not left behind. A failed pixmap load discards the candidate snapshot and its temporary directory before returning the neutral hidden-preview state, so an unusable snapshot cannot pin generated files. Canonical preview generation computes one layout plan per refresh and reuses it for the generated `full`, `text`, and `stamp` roles; it does not imply concurrent QtPdf loads. The helper returns widget-facing render state rather than mutating widgets directly.
- Status: Confirmed by code and tests.
...
|---|---|---|---|---|
| `SigningRequest` | `domain/models.py` | Public signing command payload. | input/output PDF paths, certificate path/passphrase, optional TSA URL and timestamp flag, trust policy, optional visible signature rect/appearance. | Visible signature settings must be a complete pair; `tsa_url` is required only when `timestamp_required=True`, allowing offline V1 signing without a TSA. |
...
The live document placement interaction is owned by `viewer_widget.py` and is composed through the
typed workspace boundary rather than by the AppFrame or signing-properties panel. The viewer owns one
`PlacementHistory` instance and one authoritative interaction mode (`pan`, `signature`, or `text`).
`set_interaction_mode()` updates the viewer cursor and mode-dependent rendering; the visible rectangle
is projected in every mode, while resize handles and handle hit-testing are available only in
`signature` (Place) mode. `focus_viewer()` gives the canvas strong keyboard focus when AppFrame invokes
Place or Adjust Placement, so the viewer receives Enter, Escape, arrow, Ctrl-arrow, Delete, and
Ctrl-Z/Ctrl-Shift-Z events. The scroll container proxies these operations without owning placement
state.

Pointer and keyboard operations use separate effect paths but converge on the same typed runtime
callbacks. A new pointer drag and a handle drag acquire the viewer mouse grab; Escape clears the active
gesture, releases the grab, restores the pre-gesture geometry for an existing handle, and prevents a
later release from committing a change. Idle Escape from Place mode changes the authoritative viewer
mode to Pan while retaining the completed rectangle. Accepted pointer, keyboard, panel, and removal
mutations commit exactly once to `PlacementHistory`; overlay repaint, readiness refresh, and unchanged
panel notifications only project state and do not clear or append history. Undo/Redo replays a stored
`SignatureRect | None` through `SigningWorkspaceRuntime.apply_keyboard_placement()` without creating a
new history entry. The runtime and `SigningWorkspaceSessionPort` expose required capability and action
...
Cinnamon/X11 human Gate 2 item 10 retest for held movement, Ctrl/Ctrl+Shift resize, one-step
Undo/Redo, responsiveness, and sustained CPU/disk behavior.
...
| The properties-panel surface still contains a broad Qt adapter, while AppFrame and certificate-management edges retain explicit dialog compatibility exposure. | Shell callers must not reach through `_setup_session`, and dialog tests must not couple to ephemeral panel state; some older frame/certificate callers still consume their owning compatibility fields. The AppFrame source-recovery path now uses one typed `WorkspaceHandle` capability, but its narrow `current_*` read properties remain intentional seams. | `signing_workspace_setup_port.py` provides typed setup capabilities, `signing_workspace_refinement_dialog.py` owns modal construction, and refinement tests inject the existing typed observer through a test-local subclass. AppFrame dialog properties and certificate dialog compatibility fields remain named, intentional surfaces. | Migrate any remaining AppFrame/certificate compatibility consumers in their owning plans; do not reintroduce a panel-held refinement dialog reference or duplicate active-workspace lookups. |
...
| Placement editing is numeric-only in the reusable Library detail surface, while the live document viewer owns pointer, keyboard, mode, cancellation, and placement history behavior. | Users can create and edit fixed-page placement profiles numerically; in an open document, pointer drag creates a page-local rectangle, handle drags resize it, keyboard focus and exact movement/resizing are routed to the canvas, Escape cancels gestures or returns to Pan, and placement mutations support Undo/Redo. The installed Cinnamon/X11 Gate 2 item 10 acceptance passed for package `f800e6c...` from commit `8903512c9`; Ctrl+Y remains a future UX opportunity, while certificate create/import/configure stays within its dedicated surface. | `PlacementEditorState`/`PlacementEditorSession` keep reusable edits transactional, `PlacementProfileEditorDialog` exposes exact numeric fields plus Save/Cancel, and `viewer_widget.py` owns the authoritative interaction mode, strong focus, mouse-grab cancellation, visible overlay, handle projection, and `PlacementHistory`; `signing_workspace_runtime.py` and `SigningWorkspaceSessionPort` provide the typed callback/capability boundary, while AppFrame keeps native text Undo separate from placement Undo. `ReusableObjectLibraryDialog` provides typed reusable-object duplicate/pin/rename/delete routing, confirmation-safe destructive actions, merged configured/unconfigured certificate rows with typed certificate pin/rename/delete routing, refresh/reselection after retained-file Configure, pinned-first Name A-Z/Z-A/Expiration-soonest projection, and persisted catalog/sort preferences. | Preserve the passing installed behavior and consider Ctrl+Y only in a separate UX decision; keep certificate create/import/configure inside the dedicated Settings certificate surface, nested rich editors, and dirty-detail prompts as explicit deferred boundaries; do not duplicate application policy in Qt. |
...
| 2026-08-16 | Added bounded source-tree X11 visual/geometry evidence. | `scripts/live_gui_accessibility_audit.py --capture-screenshot` now records primary-screen geometry, DPR/DPI, widget rectangles, and captures the exact Qt-owned client widget rather than delegating to a desktop screenshot helper. The inspected 1100x700 no-document frame showed the menu, message, and full-width Open/Library controls on `DP-4`; native F1 and cleanup remained green. This is concrete X11 evidence only: human assistive-technology, contrast, physical-DPI/monitor interpretation, privileged installation, final release, and Wayland remain open/deferred. |
| 2026-08-16 | Reconciled active UI plan publication markers. | Current commits and focused tests prove the non-blocking signing transaction, explicit confirmation/output policy, Document Signatures review, and per-signature guidance slices are implemented; stale commit/dependency markers were corrected in their ExecPlans. This changes status documentation only. Human/display accessibility, privileged installation, final release, and Wayland remain open/deferred. |
| 2026-08-16 | Added and corrected the optional X11 AT-SPI audit boundary. | `scripts/x11_atspi_probe.py` is a host-Python, read-only, PID/title-owned probe invoked only by `live_gui_accessibility_audit.py --probe-atspi`; the audit captures `org.a11y.Bus`/status before constructing `QApplication`, supports the forced `QT_LINUX_ACCESSIBILITY_ALWAYS_ON=1` backend option, resolves the dedicated AT-SPI address for `pyatspi`, and preserves bounded child return-code, timeout, and stderr diagnostics. Controlled normal and forced FoliaSeal runs, an Orca-active forced run, and a minimal-Qt control all reached the bridge but timed out during bounded discovery (with missing AT-SPI socket warnings and Qt's `GetApplicationBusAddress` warning), while native F1 and cleanup passed. This is session/bridge-wide evidence rather than a FoliaSeal-specific accessibility failure; no accessible-tree or screen-reader claim is made. |
...
| 2026-09-06 | Hardened the signing-action transaction boundary after a live confirmation no-op. | `SigningActionBoundary` now exposes explicit begin/poll/close entry points, converts missing-runner and worker-start failures into visible terminal `sign_failure` results, and preserves status/error delivery for the shell. Native confirmation and asynchronous bridge routes have focused real-Qt coverage; the installed-package signing retest remains open. |
| 2026-09-06 | Aligned signing-request validation with offline-first V1 policy. | `SigningRequest` accepts an empty `tsa_url` when `timestamp_required=False`, while retaining non-empty URL validation for explicitly timestamped requests. This matches `SPEC.md`'s offline core workflow and keeps timestamping outside the normal GUI path. |
...
| 2026-08-21 | Completed the source-tree Cinnamon/X11 GUI audit and rail reflow correction. | The live audit now records 22 rendered/keyboard checkpoints through the two-signature workflow, including menus, dialogs, placement editing, reopen/verify, and native Help delivery. Rail-owned controls shrink to the actual viewport, preview geometry reapplies after resize, and status-detail labels stay inside the card; focused regression coverage protects these constraints. Screen-reader speech, high-contrast/DPI perception, installed-package behavior, and fixture-limited signed/restricted/multi-page states remain separate acceptance gates. |
| 2026-08-22 | Removed schema terminology from ordinary GUI copy. | Qt menus, certificate dialogs, signing readiness/errors, the signing rail, refinement controls, and Appearance validation now use Certificate, Certificate file, Appearance, and Placement. Persisted schema identifiers and catalog keys remain unchanged; a terminology regression guard covers the presentation boundary and direct GUI message producers. The rendered audit adds a Certificate-management checkpoint; screen-reader, high-contrast/DPI, packaging, and fixture-limited gates remain separate acceptance work. |
...
| 2026-08-16 | Reconciled product-support and release status. | Packaged Help, privacy-safe diagnostics, Restore defaults, offscreen real-Qt accessibility, and isolated package-manager installation are implemented and evidenced. Screen-reader/high-contrast/DPI/monitor, display-backed GUI, privileged host installation, and final release acceptance remain external gates; no source ownership changed. |
| 2026-08-16 | Audited remaining compatibility and nomenclature retirement candidates. | No `phase3` references remain in product source, scripts, or tests. Active `Acceptance*` CLI/DTO/JSON/artifact contracts and the test-consumed `build_qt_signing_shell` adapter remain intentionally retained; `PdfCompatibility` is production signing policy. No safe additional deletion was identified. |
| 2026-08-16 | Repeated the supported X11 frame audit at Qt device-pixel ratio 2.0. | The exact Qt-owned frame retained its 1100x700 logical geometry, two-screen context, native F1, and clean teardown; direct inspection found the menu, empty state, Open, and Library controls unclipped. The desktop exposed only 960x540 available logical geometry, so whole-window monitor fit/restoration remains open. This is geometry evidence only and does not claim physical readability, screen-reader behavior, privileged packaging, final release acceptance, or Wayland support. |
| 2026-08-16 | Completed the canonical parent signing workflow audit on Cinnamon/X11. | `scripts/live_gui_parent_audit.py` completed 19 real-display checkpoints for setup, reusable objects, placement, asynchronous signing, reopen, and a second signature; the retained outputs locally verified with one and two signatures. The audit now resolves the concrete shell behind the opaque workspace view, waits for the real transaction result, uses consequence-labeled confirmation buttons, and discards its owned draft before teardown. The coordinator preserves an unchanged selected preset during confirmation synchronization and still clears it after an actual edit. Wayland, human accessibility/DPI/monitor, packaged GUI, privileged installation, and final release gates remain open. |
| 2026-08-16 | Added typed partial-preset missing-input readiness guidance. | `signature_properties_coordinator.py` derives selected-preset certificate/placement omissions, `signing_readiness.py` projects them as `SigningReadinessMissingInput`, and the Qt panel renders certificate-first, placement-second guidance without mutating certificate selection, placement, or signing state. `NO_CERTIFICATE_SELECTED` is treated as the expected partial omission; blocking certificate statuses retain their detailed errors. |
| 2026-08-16 | Added nested blank-page Placement creation to the Preset editor. | `SignaturePresetEditorWidget` invokes the existing AppFrame `PlacementProfileEditorDialog` boundary, receives a persisted `PlacementProfile`, refreshes only the suspended draft's Placement selector, and attaches the stable id on explicit Preset Save. Catalog changes remain separate from the active signing workflow; Certificate creation/import and current-document capture remain follow-up boundaries. |
| 2026-08-16 | Added nested Certificate creation/import to the Preset editor. | The suspended Preset now exposes explicit Create/Import actions that reuse AppFrame's existing certificate dialogs, refresh the provider-backed catalog, attach only the returned `CertificateConfiguration` stable id, and leave active signing state unchanged. |
| 2026-08-16 | Added nested current-document Placement capture to the Preset editor. | The explicit Capture action crosses typed `current_placement_context()`/`signature_rect()` session reads, reuses `PlacementEditorState.from_current_page()` for visible-page conversion, persists a document-independent `PlacementProfile`, and attaches only its stable id on explicit Preset Save. The retained no-document Library reports an error at invocation. Focused validation is `123 passed`; display-backed, privileged, final-release, and Mint 22.3 Wayland gates remain open/deferred. |
| 2026-08-16 | Added active reusable-object placement invalidation. | `ReusableSigningObjects` now classifies material Appearance/Placement/Preset edits and deletes by stable reference before catalog persistence; the AppFrame compares that reference with the active placed draft and asks `Remove the placed signature and continue?` with safe Cancel default. Cancel preserves both catalog and placement, while Yes removes placement through the typed session seam. Rename, pin, duplicate, and unrelated-object changes remain nonmaterial. |
...
| 2026-08-10 | Added the document-independent Signature Preset editor. | Historical precursor: `SignaturePresetEditorDialog` first established the isolated stable-reference Save/Cancel contract; production Create/Edit now uses the nested `SignaturePresetEditorWidget` path documented above, while the dialog remains a compatibility/test wrapper. Reason/location defaults and final preview fidelity remain deferred; active-placement invalidation was deferred at this historical checkpoint and is implemented by the 2026-08-16 mutation-boundary slice. |
...
| 2026-08-09 | Reconciled the fixed signing rail and protected status-region correction. | Historical baseline: `SigningWorkspaceSidebar` owned a fixed 320px rail and a 200px protected status floor. The current surface retains the fixed rail and read-only separation but uses content-driven status sizing; `SigningActionState.recommended_action` remains typed and visibly rendered. |
| 2026-08-23 | Reconciled document-review rail density and explicit save-path entry. | The sidebar retires duplicate search-copy/hidden selection mirrors, hides empty signature-selector rows while preserving restricted guidance, collapses empty status labels, and keeps the primary `Confirm and sign` action full-width. Direct primary signing requests Save As before confirmation when no explicit destination exists; cancellation is lossless. |
...
| 2026-08-09 | Extended the typed app-frame command registry with Settings actions. | Historical increment: `app_frame_command_model.py` first owned the stable File/View/Settings registry with unique mnemonics, stable IDs/object names, and Qt descriptions; `FoliaSealAppFrame` owned the command-to-QAction mapping and five Settings routes. Later command-model increments completed Edit, View, Signing, and Help; the current complete registry and release limits are recorded in the command-model ExecPlan. |
