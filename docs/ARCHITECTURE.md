...
- Does not own: viewer text highlighting, arbitrary freeform selection geometry, clipboard UI, or document mutation.
...
- Known constraints: This first-pass slice supports search, page navigation to the current hit, and copy-current-hit text only. It deliberately does not implement arbitrary text selection or highlight overlays, and it must fail soft for missing or unreadable PDFs.
- Status: Confirmed by code and tests.
