...
| 2026-08-16 | Added and corrected the optional X11 AT-SPI audit boundary. | `scripts/x11_atspi_probe.py` is a host-Python, read-only, PID/title-owned probe invoked only by `live_gui_accessibility_audit.py --probe-atspi`; it preflights `org.a11y.Bus`, resolves the dedicated AT-SPI address for `pyatspi`, and records accessible-tree data only for the owned process/title. The current Cinnamon/X11 Qt frame reaches the bridge boundary but times out during bounded discovery with Qt's `GetApplicationBusAddress` warning, so no accessible-tree or screen-reader claim is made. |
