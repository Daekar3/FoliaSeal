...
- Known constraints: Widgets use dynamic PySide6 imports and many test doubles. `app_frame.py` owns a first-pass `QMainWindow` wrapper with File/Open, Settings/Application settings, Settings/Import certificate, and Settings/Manage certificate configurations actions; the settings action opens an app-wide settings dialog for default directories, the import action opens a PKCS#12 import dialog with optional secure password saving, and the management action can rename, edit notes for, or delete `CertificateConfiguration` records, export managed PKCS#12 files, and delete unreferenced `ManagedCertificate` records/files. `signing_shell.py` is a large module with control dataclasses, helper functions, properties panel, workspace widget, and shell adapter in one file. The shell can select and apply existing certificate configurations, including configurations that resolve saved passwords through the app-frame-provided secret provider, and can refresh its selector after app-frame certificate catalog changes, but certificate creation is still pending. The signing shell consumes `AppSettings` for output-path defaults but no longer edits app-wide directory settings directly.
...
- Location: `src/foliaseal/infra/config/schemas.py`, `src/foliaseal/infra/config/app_settings_storage.py`, `src/foliaseal/infra/config/profile_storage.py`, `src/foliaseal/infra/config/certificate_storage.py`, `src/foliaseal/infra/secret_storage.py`, `src/foliaseal/application/certificate_import.py`, `src/foliaseal/application/signing_material_resolver.py`
- Responsibility: Serialize, deserialize, validate, load, and save trust/timestamp configuration plus reusable signing-object catalogs; import existing PKCS#12 certificates into app-managed storage; store saved certificate passwords through secure external secret storage.
- Owns: `AppSettings`, `AppSettingsStore`, `TrustProfile`, `TimestampPolicy`, `ManagedCertificate`, `CertificateConfiguration`, `CertificateCatalog`, `CertificateCatalogStore`, `SecretToolCertificateSecretStore`, `SigningMaterial`, `CertificateSigningMaterialResolver`, `AppearanceProfile`, `PlacementProfile`, reference-only `SignaturePreset`, `ResolvedSignaturePreset`, `SignaturePresetCatalog`, `SignaturePresetCatalogStore`.
...
- Producer: `CertificateCatalogStore.save_catalog()`, `CertificateCatalogStore.delete_configuration_by_id()`, `CertificateCatalogStore.delete_managed_certificate_by_id()`, `CertificateCatalogStore.export_managed_certificate_by_id()`, and `CertificateImportService.import_pkcs12()` through the app-frame certificate dialogs. Saved certificate password values are produced only to the external secret provider, not to the JSON catalog.
- Consumer: Qt app-frame certificate import/management, signing-material resolver, Qt signing shell certificate selector, and future launches.
- Stability: Persisted file contract.
...
- Source files: `src/foliaseal/infra/config/schemas.py`, `src/foliaseal/infra/config/certificate_storage.py`, `src/foliaseal/infra/secret_storage.py`, `src/foliaseal/application/certificate_import.py`, `src/foliaseal/application/signing_material_resolver.py`, `src/foliaseal/presentation/qt/app_frame.py`.
...
| Managed certificate configuration | Certificate import service, app-frame certificate management dialog, tests, or Qt shell selection | imported PKCS#12 -> managed certificate record + certificate configuration -> optional `secret-tool` password save -> optional app-frame rename/notes/delete of configuration record -> optional app-frame export of managed certificate file -> optional app-frame delete of unreferenced managed certificate/file -> resolver -> runtime signing material -> signing draft | XDG data dir under `FoliaSeal/Certificates/certificates.json`; PKCS#12 files under `FoliaSeal/Certificates/Managed/`; saved passwords in Linux Secret Service via `secret-tool` | `CertificateCatalog` JSON plus PKCS#12 files plus external secret references | JSON stores secret references only; no plain certificate passwords. Qt app frame can import existing PKCS#12 files, optionally save passwords securely when `secret-tool` is available, rename/edit notes/delete configuration records, export managed certificate files, delete unreferenced managed certificate files, and Qt shell can select existing configurations; in-app certificate creation remains pending. |
...
| Full certificate management is not implemented yet. | Users can import existing PKCS#12 files, optionally save passwords through `secret-tool`, rename/edit notes/delete resulting certificate configurations, export/back up managed certificate files, delete unreferenced managed certificate files, and select configurations, but they cannot yet create certificates in-app. | Tests and lower-level stores can create catalogs; the app frame owns first-pass import/configuration/export/managed-certificate deletion and saved-password cleanup, and the shell consumes configurations through the resolver. | Add in-app certificate creation and consider cross-platform credential-store adapters if FoliaSeal expands beyond Linux Secret Service. |
