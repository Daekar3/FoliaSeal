...
def test_qt_acceptance_harness_workspace_adapter_applies_scenario_and_syncs_viewer() -> None:
...
def test_qt_acceptance_harness_workspace_adapter_prefers_dedicated_testing_adapter() -> None:
    command = InteractiveHarnessScenarioCommand(
        profile_name=None,
...
def test_qt_acceptance_harness_workspace_adapter_refreshes_viewer_directly() -> None:
    class _FakeTestingAdapter:
        def __init__(self) -> None:
            self.viewer_refreshes = 0
...
def test_qt_acceptance_harness_workspace_adapter_requires_typed_workspace() -> None:
    with pytest.raises(TypeError):
        QtAcceptanceHarnessWorkspaceAdapter(profile_store=object())


def test_qt_acceptance_harness_workspace_adapter_returns_snapshot_with_request_and_result() -> None:
    preview = type(
        "_Preview",
        (),
        {
    ...
    adapter = QtAcceptanceHarnessWorkspaceAdapter(
        ...
        deps=QtAcceptanceHarnessWorkspaceDeps(
            capture_preview_render=QtPreviewRenderCaptureAdapter(callback=capture_preview_render),
...
def test_capture_qt_preview_render_preserves_gui_preview_and_bordered_analysis_preview(
    ...
    def _fake_render(preview, **kwargs):
        ...
        return type(
            ...
            (),
            {
                ...
                "appearance_snapshot": interactive_harness_module.SignatureAppearanceSnapshot(
                    image_path=str(analysis_path),
                        ...
                        (),
                        {
                            "_canonical_preview_snapshot": type(
                                ...
                                (),
                                {
                                    ...
                                    "stamp_bounds_px": None,
                                    "appearance_snapshot": (
                                        interactive_harness_module.SignatureAppearanceSnapshot(
                                            image_path=str(gui_path),
    ...
    monkeypatch.setattr(interactive_harness_module, "_widget_is_visible", lambda widget: True)
    monkeypatch.setattr(
        interactive_harness_module,
    ...
    monkeypatch.setattr(interactive_harness_module, "_label_alignment_snapshot", lambda label: None)
    monkeypatch.setattr(
        interactive_harness_module, "_label_pixmap_size_snapshot", lambda label: None
    )
    monkeypatch.setattr(interactive_harness_module, "_layout_spacing", lambda layout: 0)
    monkeypatch.setattr(interactive_harness_module, "_size_hint_snapshot", lambda widget: None)
    monkeypatch.setattr(
        interactive_harness_module,
    ...
    monkeypatch.setattr(
        interactive_harness_module, "_write_text_debug_overlay", lambda **kwargs: None
    )
    monkeypatch.setattr(
        interactive_harness_module, "_write_stamp_debug_overlay", lambda **kwargs: None
    )
    ...
    capture = capture_qt_preview_render(
        ...
        artifact_basename="interactive_state_01",
        build_preview_render_capture_payload=(
            interactive_harness_module._build_qt_preview_render_capture_payload
        ),
    )
...
def test_qt_acceptance_harness_workspace_adapter_rejects_missing_testing_adapter() -> None:
    with pytest.raises(TypeError):
        QtAcceptanceHarnessWorkspaceAdapter(profile_store=object())


def test_headless_acceptance_harness_workspace_adapter_applies_same_scenario_fields() -> None:
...
def test_headless_acceptance_harness_workspace_adapter_captures_preview_state() -> None:
    ...
    adapter = HeadlessAcceptanceHarnessWorkspaceAdapter(
        workflow=workflow,
        profile_store=object(),
...
def test_headless_acceptance_harness_workspace_adapter_refresh_viewer_is_no_op() -> None:
    request = build_signing_request(Path("/tmp"))
    workflow = SigningDraftWorkflow.from_signing_request(request)
    adapter = HeadlessAcceptanceHarnessWorkspaceAdapter(
        workflow=workflow,
