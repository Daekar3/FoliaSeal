...
def test_signing_executor_produces_signed_pdf_and_validates(tmp_path: Path) -> None:
...
def test_prepared_signing_plan_contains_one_visible_layout_result(tmp_path: Path) -> None:
    ...
    prepared = prepare_signing_plan(backend_request)
...
def test_prepared_signing_plan_converts_layout_issues_to_application_issues(
    ...
    prepared = prepare_signing_plan(SigningBackendRequest.from_signing_request(request))
...
def test_signing_executor_supports_invisible_incremental_signing(
    tmp_path: Path,
) -> None:
    input_pdf = tmp_path / "input.pdf"
    output_pdf = tmp_path / "output.pdf"
...
def test_signing_executor_produces_visible_signature_without_image_stamp(
    tmp_path: Path,
) -> None:
    input_pdf = tmp_path / "input.pdf"
    output_pdf = tmp_path / "output.pdf"
...
def test_signing_executor_appends_second_incremental_signature(
    tmp_path: Path,
) -> None:
    input_pdf = tmp_path / "input.pdf"
    ...
    first_result = build_signing_executor().execute(first_request)
    ...
    second_result = build_signing_executor().execute(second_request)
...
def test_signing_executor_adds_timestamp_when_required_with_tsa(
    tmp_path: Path,
) -> None:
    input_pdf = tmp_path / "input.pdf"
    output_pdf = tmp_path / "output.pdf"
...
def test_signing_executor_reports_trusted_timestamp_when_anchors_are_configured(
    tmp_path: Path,
) -> None:
...
def test_horizontal_single_line_backend_validation_uses_ink_reference_for_compact_stamp_lane(
    ...
    monkeypatch.setattr(
        "foliaseal.application.signing_backend._DEFAULT_RENDERED_FIT_PROBE.single_line_fits",
...
def test_signing_executor_signs_compact_single_line_rectangle(
    tmp_path: Path,
) -> None:
    input_pdf = tmp_path / "input.pdf"
    output_pdf = tmp_path / "output.pdf"
...
def test_signing_executor_keeps_compact_vertical_stamp_visible(
    tmp_path: Path,
    stamp_position: SignatureStampPosition,
) -> None:
    input_pdf = tmp_path / "input.pdf"
...
def test_signing_executor_maps_wrong_password_to_stable_failure(
    tmp_path: Path,
) -> None:
    _write_test_pdf(tmp_path / "input.pdf")
    _write_test_pkcs12(tmp_path / "cert.p12", passphrase="secret")
...
def test_signing_executor_fails_honestly_when_timestamp_is_required(
    tmp_path: Path,
) -> None:
    _write_test_pdf(tmp_path / "input.pdf")
    _write_test_pkcs12(tmp_path / "cert.p12", passphrase="secret")
...
def test_acceptance_certificate_loader_accepts_valid_pkcs12(tmp_path: Path) -> None:
    cert_path = tmp_path / "cert.p12"
    _write_test_pkcs12(cert_path, passphrase="secret")
...
def test_acceptance_certificate_loader_rejects_malformed_pkcs12(tmp_path: Path) -> None:
...
def test_signing_executor_rejects_visible_signature_that_does_not_fit(
    tmp_path: Path,
) -> None:
    _write_test_pdf(tmp_path / "input.pdf")
    _write_test_pkcs12(tmp_path / "cert.p12", passphrase="secret")
...
def test_single_line_rendered_ink_fallback_caches_identical_checks(
    ...
    monkeypatch.setattr(
        "foliaseal.application.signing_backend.detect_text_content_bounds_in_image",
