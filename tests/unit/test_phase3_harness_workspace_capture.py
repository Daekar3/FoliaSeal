...
from foliaseal.presentation.qt.phase3_harness_workspace_capture import (
    Phase3HarnessWorkspaceCaptureInput,
    Phase3HarnessWorkspaceCaptureService,
    Phase3HarnessWorkspaceSnapshot,
)


def _capture_input(*, label: str | None = None) -> Phase3HarnessWorkspaceCaptureInput:
    return Phase3HarnessWorkspaceCaptureInput(
...
def test_capture_service_preserves_mapping_and_optional_label() -> None:
    snapshot = Phase3HarnessWorkspaceCaptureService().build_snapshot(
...
def test_capture_service_omits_empty_label_without_changing_fields() -> None:
    snapshot = Phase3HarnessWorkspaceCaptureService().build_snapshot(_capture_input())
...
def test_capture_module_import_is_free_of_optional_runtime_dependencies() -> None:
    code = (
        "import sys; "
        "import foliaseal.presentation.qt.phase3_harness_workspace_capture; "
