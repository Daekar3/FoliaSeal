...
from foliaseal.presentation.qt.evidence_interactive_capture import InteractiveHarnessCapture
...
def test_finalize_interactive_harness_report_evaluates_renders_and_writes() -> None:
    calls = {"writes": []}

    def fake_evaluator(payload):
        calls["payload"] = payload
        return SimpleNamespace(contract_version="evidence_v1")
    ...
    result = finalize_interactive_harness_report(
        InteractiveHarnessReportRequest(
    ...
    assert calls["writes"] == [
        (
            ...
            '{"contract_version": "evidence_v1", "pdf_path": "/tmp/sample.pdf"}\n',
        ),
        ("artifacts/results.md", "rendered checklist"),
    ]
    assert result.capture.summary_json_path == "artifacts/summary.json"
    assert result.contract.contract_version == "evidence_v1"
    assert result.checklist_results == "rendered checklist"


def test_acceptance_checklist_results_markdown_auto_checks_supported_items(
    tmp_path: Path,
) -> None:
    template = tmp_path / "acceptance_template.md"
    template.write_text(
        "\n".join(
            [
                (
    ...
    markdown = build_acceptance_checklist_results_markdown(
        capture,
        checklist_template_path=str(template),
    )

    assert "Acceptance FR-3B Acceptance Results" in markdown
    assert "- Acceptance tier: `gate_candidate`" in markdown
    assert "- Automated gate verdict: `gate_candidate`" in markdown
    assert "- Evidence contract version: `evidence_v1`" in markdown
...
def test_acceptance_checklist_results_markdown_leaves_manual_items_unchecked(
    tmp_path: Path,
) -> None:
    template = tmp_path / "acceptance_template.md"
    template.write_text(
